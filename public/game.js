...
const CELL_W = 92, CELL_H = 94;
const GRID_X = 64, GRID_Y = 14;
const W = GRID_X + COLS * CELL_W + 12;   // 1088
// 战场右边界：敌人越过这条线才算「上场」。
// 在这之前双方都不能互相攻击——否则火力会在出生点就把敌人打光，前线永远推不起来。
const FIELD_X = GRID_X + COLS * CELL_W;  // 1076
const H = GRID_Y + ROWS * CELL_H + 12;   // 590
...
const MAPS = {
  ...
  tomb: {
    ...
    sky: ['#6b4a22', '#4a3117', '#2b1c0d'],
    cell: [['#7a5c30', '#654a25'], ['#725429', '#5d4220']],
    wall: ['#8a6a34', '#3f2d14'], rivet: 'rgba(255,225,150,0.45)',
    sand: true,
    boss: 'pharaoh',
    tip: '五条全是沙道，但法老会抽走你的电量给自己变护卫',
  },
  forge: {
    name: '熔岩工厂', icon: '🌋', lanes: 'GGLLGG',
    sky: ['#3a1410', '#25100e', '#160a09'],
    cell: [['#3b2420', '#2e1b18'], ['#341f1c', '#281714']],
    wall: ['#5a2c1e', '#2a1410'], rivet: 'rgba(255,170,110,0.4)',
    water: ['rgba(214,72,20,0.92)', 'rgba(158,40,12,0.94)', 'rgba(96,20,8,0.96)'],
    foam: 'rgba(255,214,130,0.75)', caustic: 'rgba(255,190,90,0.3)',
    ember: true,
    boss: 'forgegolem',
    tip: '中间两条是岩浆河：踩得住的只有会飞的；地雷与钉刺放不进去',
  },
  tundra: {
    name: '极地冰原', icon: '❄️', lanes: 'GGWWGG',
    sky: ['#1c3350', '#16283f', '#0f1b2b'],
    cell: [['#2e4a66', '#25405a'], ['#2a4460', '#213a53'], ],
    wall: ['#35597c', '#182b3f'], rivet: 'rgba(220,240,255,0.5)',
    water: ['rgba(120,196,224,0.8)', 'rgba(72,150,190,0.86)', 'rgba(36,96,140,0.92)'],
    foam: 'rgba(240,252,255,0.7)', caustic: 'rgba(225,248,255,0.24)',
    snow: true,
    boss: 'frostbehemoth',
    tip: '中间两条是裂开的冰河；风雪里所有敌人都抗冻',
  },
  abyss: {
    name: '终焉深渊', icon: '🌑', lanes: 'SSGWGS',
    sky: ['#1b1030', '#140b24', '#0b0716'],
    cell: [['#2a1c47', '#22163a'], ['#251940', '#1d1333']],
    wall: ['#3a2560', '#1a1030'], rivet: 'rgba(201,138,255,0.45)',
    water: ['rgba(96,44,160,0.86)', 'rgba(62,26,112,0.9)', 'rgba(34,14,64,0.94)'],
    foam: 'rgba(216,170,255,0.6)', caustic: 'rgba(201,138,255,0.22)',
    ember: true,
    boss: 'voidsovereign',
    tip: '上面悬空、中间一条虚空之海 —— 六条通道三种地形，什么都来',
  },
  skyport: {
...
const STYLE_TIP = {
  box: '花 50⚡ 开盲盒，开出什么用什么',
  classic: '全部机器明码标价，按冷却部署',
  creative: '能量无限、免费无冷却，随便实验',
};
const MAP_ORDER = ['scrapyard', 'pool', 'beach', 'tomb', 'forge', 'tundra', 'skyport', 'abyss'];
const STAGES_PER_MAP = 10;          // 每张图 4 关，第 4 关打 Boss
// 沉底 / 没有地面就放不了的机器
...
try { const v = localStorage.getItem('mg_campStyle'); if (v && STYLE_NAME[v]) campStyle = v; } catch (e) {}
...
const KING_HOLD_COL = COLS - 3;               // 最后三列
function kingHoldX() { return GRID_X + KING_HOLD_COL * CELL_W + CELL_W * 0.5; }
// 下一关是谁：同一张图往后数，数完了跳下一张图的第 1 关；全通了返回 null
function nextStage(map, st) {
  if (st < STAGES_PER_MAP) return { map, st: st + 1 };
  const i = MAP_ORDER.indexOf(map);
  if (i >= 0 && i < MAP_ORDER.length - 1) return { map: MAP_ORDER[i + 1], st: 1 };
  return null;
}
// 一路打下去用的：已解锁的最远一关
...
// 战役过关：存进度，然后直接推进到下一关（一张图打完就跳下一张图），
// 不再退回菜单——玩家要的是一路打下去。
...
function clearStage() {
  ...
  const justUnlocked = wasLocked && allCardsUnlocked();
  const nx = nextStage(curMap, curStage);
  if (!nx) { endGame(true); return; }   // 全部通关：正常结算
  state = 'win';
  closeDeck();
  sel = null;
  renderTray();
  advanceTo = nx;
  advanceT = 3.2;
  const sameMap = nx.map === curMap;
  $('clearTitle').innerHTML = isBossStage()
    ? '☠ <span class="gold">' + ENEMIES[mapDef().boss].name + ' 已被击碎！</span>'
    : '🎉 <span class="gold">第 ' + curStage + ' 关 通过！</span>';
  if (justUnlocked) {
    $('clearTitle').innerHTML = '🔓 <span class="gold">全部机器解锁！</span>';
  }
  $('clearSub').textContent = justUnlocked
    ? '打通了 ' + MAPS[STARTER_MAP].name + '，卡槽里的机器全开了 —— 下一站：'
      + MAPS[nx.map].icon + ' ' + MAPS[nx.map].name
    : sameMap
    ? '下一关：' + mapDef().name + ' 第 ' + nx.st + ' 关'
    : '新地图解锁：' + MAPS[nx.map].icon + ' ' + MAPS[nx.map].name + ' —— ' + MAPS[nx.map].tip;
  show('clear', true);
  sfx('win');
}
...
/* ===== 地形是按「格」算的 =====
   整行一种地形用 lanes 写（每行一个字符）；
   竖着的一条水道用 waterCols 写（列号）—— 落日海滩右边那条就是这么来的。
   'L'（岩浆）在规则上等同于水，分成两个字符只是为了让背景画得不一样。 */
function laneRaw(r) { return mapDef().lanes[r] || 'G'; }
function laneOf(r) { const L = laneRaw(r); return L === 'L' ? 'W' : L; }
...
function waterOk(type, row, col, modules) {
  ...
  return hasPad(row, col);
}
// 这一格能不能放这台机器（地雷钉刺只认实地）
function rowAllows(type, r, c) {
  ...
  if (terrainAt(r, c) === 'G') return true;
  return !mods.some(x => GROUND_ONLY.indexOf(x.kind) >= 0);
}
// 这一行走不走得了这种敌人
function rowPassable(info, r) {
  const L = laneOf(r);
  if (info.swim) return rowTouchesWater(r);  // 会游的：这一行有水就来得了
  if (L === 'W') return !!info.fly;          // 整行都是水：只有会飞的过得去
  return true;                               // 地面 / 悬空 / 被竖水道穿过的行：都能走
}
/* 蹚水：地面单位走进水格会被拖慢，水生单位爬上岸也一样难受。
   竖着的那条水道就是这么变成天然屠宰场的。 */
function wadeMul(e) {
  if (e.fly) return 1;
...
const MACHINES = {
  ...
  scrapult:  { name: '废铁投石机', rarity: 'fusion', hp: 480, desc: '把绞碎的废铁当炮弹甩出去：曲射落点范围更大，还会崩碎装甲' },
  rivetgun:  { name: '铆钉机枪',   rarity: 'fusion', hp: 520, desc: '打出的铆钉会钉在地上变成钉刺，越打越密' },
  torpedo:   { name: '鱼雷艇坞',   rarity: 'fusion', hp: 380, desc: '贴着水面放鱼雷，穿透整行；对会游、会飞的都加伤' },
  conch:     { name: '号角贝垒',   rarity: 'fusion', hp: 1700, desc: '一堵会吹号的厚壳墙：挡得住，还把撞上来的敌人震回去' },
  moltengun: { name: '熔铁机炮',   rarity: 'fusion', hp: 420, desc: '打熔铁弹：命中即点燃，火焰还会溅到旁边' },
  icespire:  { name: '冰晶尖塔',   rarity: 'fusion', hp: 780, desc: '塔下结一片冰刺地：踩上来的敌人又慢又被扎' },
  skylance:  { name: '天穹长枪',   rarity: 'fusion', hp: 320, desc: '对空激光矛：贯穿整行，专打飞行单位' },
  voidprism: { name: '虚空棱镜',   rarity: 'fusion', hp: 330, desc: '棱光分裂三行，命中处再塌出一个小引力井' },
  obeliskgun:{ name: '方尖碑炮',   rarity: 'fusion', hp: 460, desc: '一边打贯穿光柱一边产电——法老抽走多少，它就给你挣回来多少' },
  sandworm:  { name: '沙虫巢',     rarity: 'fusion', hp: 520, desc: '沙下养着一条虫：埋雷，还会把爬到跟前的敌人整个吞掉' },
  junkcoil:  { name: '废铁电磁桩', rarity: 'fusion', hp: 400, desc: '电磁桩：把本行敌人往回拽的同时过一遍电' },
  icetrawler:{ name: '结冰拖网',   rarity: 'fusion', hp: 480, desc: '网住飞行单位拖到地面，落地那一下顺手把它冻上' },
  tidehorn:  { name: '浊浪号角',   rarity: 'fusion', hp: 420, desc: '把毒浪吹出去：震退一片，还留一身毒' },
  brazier:   { name: '炙铁火盆',   rarity: 'fusion', hp: 820, desc: '烧红的铁栅：敌人能踩过去，但会被扎穿又被烫穿' },
  auroradome:{ name: '极光护穹',   rarity: 'fusion', hp: 460, desc: '给周围机器罩护盾，同时一波脉冲把本行敌人瘫痪' },
  skyhive:   { name: '天穹蜂巢',   rarity: 'fusion', hp: 400, desc: '放无人机，还给周围机器加速——蜂群越打越快' },
  voideye:   { name: '虚空瞳',     rarity: 'fusion', hp: 380, desc: '跨行狙击最肥的目标，同时拦下本行飞来的炮弹' },
};
...
/* ===== 机器解锁 =====
   ...
   地图专属卡另算：它们跟着地图走，不进这个解锁池。 */
const STARTER_CARDS = ['generator', 'turret', 'barricade', 'spikes', 'puncher',
                       'mine', 'fan', 'shredder', 'aa', 'mortar'];
const STARTER_MAP = 'scrapyard';
...
function cardUnlocked(type) {
  ...
  if (STARTER_CARDS.indexOf(type) >= 0) return true;
  return allCardsUnlocked();
}

/* ===== 关卡难度 =====
   同一关可以按三档打：怪的血量、数量和得分一起变。 */
const DIFFS = {
  easy:   { name: '轻松', hp: 0.7,  count: 0.75, score: 0.7, tip: '敌人更少更脆，适合熟悉地形' },
  normal: { name: '普通', hp: 1,    count: 1,    score: 1,   tip: '标准配置' },
  hard:   { name: '困难', hp: 1.45, count: 1.3,  score: 1.8, tip: '又多又硬，得分也翻着涨' },
};
let campDiff = 'normal';
function diff() { return DIFFS[campDiff] || DIFFS.normal; }
function saveDiff() { try { localStorage.setItem('mg_campDiff', campDiff); } catch (e) {} }
...
const PAIR_TYPE = [
  ...
  // ---- 各地图的招牌融合机：卡槽里买得到（只在自己那张图），
  //      在别的图上把对应的两张卡杂交也能做出来 ----
  [['mortar', 'shred'], 'scrapult', '废铁投石机'],
  [['shot', 'spikes'], 'rivetgun', '铆钉机枪'],
  [['rocket', 'aa'], 'torpedo', '鱼雷艇坞'],
  [['sonic', 'armor'], 'conch', '号角贝垒'],
  [['flame', 'shot'], 'moltengun', '熔铁机炮'],
  [['frost', 'spikes'], 'icespire', '冰晶尖塔'],
  ...
  [['flame', 'spikes'], 'brazier', '炙铁火盆'],
  [['emp', 'shield'], 'auroradome', '极光护穹'],
  [['drone', 'booster'], 'skyhive', '天穹蜂巢'],
  [['sniper', 'deflect'], 'voideye', '虚空瞳'],
];
...
const ENEMIES = {
  ...
  // ---- 熔岩工厂 ----
  slagcrawler: { name: '熔渣爬虫', hp: 300, speed: 13, dmg: 60, score: 60, w: 50, swim: true, lava: true },
  forgeknight: { name: '铸铁骑士', hp: 640, speed: 8,  dmg: 90, score: 95, w: 58, shield: 420, heavy: true },
  // ---- 极地冰原 ----
  glacier:    { name: '冰川推土机', hp: 1600, speed: 6, dmg: 200, score: 120, w: 84, heavy: true,
                coldResist: 3, chill: 1.4 },
  snowdrone:  { name: '暴雪无人机', hp: 210, speed: 14, dmg: 40, score: 70, w: 46, fly: true,
                coldResist: 3, range: 2.8, rdmg: 30, rcd: 2.0, rkind: 'jolt', chill: 1.2 },
  // ---- 终焉深渊 ----
  ...
  // 法老变出来的护卫：血厚、走得慢，专门用来堵在法老前面
  sentinel:  { name: '黄金石像',   hp: 1500, speed: 7, dmg: 150, score: 60, w: 62, heavy: true, shield: 300 },
...
/* 战役只决定「在哪打」，玩法还是那三种。
   campStyle 说的是这一局按哪种经济跑：盲盒抽、普通种卡、创造随便放。
   creative() / boxMode() 一律走这两个函数，别的地方不再直接比 mode。 */
let campStyle = 'classic';
const campaign = () => mode === 'campaign';
const creative = () => mode === 'creative' || (campaign() && campStyle === 'creative');
const boxMode = () => mode === 'box' || (campaign() && campStyle === 'box');
...
function applyModeUI() {
  const cardTray = !boxMode();   // 普通/创造玩法使用卡槽
  $('boxBtn').style.display = cardTray ? 'none' : '';
  $('trayInfo').style.display = cardTray ? 'none' : '';
  $('classicTray').style.display = cardTray ? '' : 'none';
  $('wavesBtn').style.display = (creative() && !campaign()) ? '' : 'none';
  $('brandMode').textContent = campaign()
    ? (STYLE_NAME[campStyle] + ' · ' + diff().name + ' · ' + mapDef().name + ' 第' + curStage + '关')
    : creative() ? '创造模式'
    : mode === 'classic' ? '普通模式' : '盲盒塔防';
  document.body.classList.toggle('boxmode', boxMode());
  if (boxMode()) closeDeck();
}
...
function renderClassicTray() {
  ...
  holder.innerHTML = '';
  const list = CLASSIC_ORDER.filter(cardAvailable);
  holder.classList.toggle('many', list.length > 10);
  classicCardEls = {};
  for (const type of list) {
    ...
    el.addEventListener('click', () => {
      if (state !== 'playing' || boxMode()) return;
...
function renderDeck() {
  ...
  grid2.innerHTML = '';
  deckEls = {};
  for (const type of CLASSIC_ORDER.filter(cardAvailable)) {
    const info = MACHINES[type];
    const el = document.createElement('div');
...
function rollType() {
  ...
  // 还没解锁的机器不该从盲盒里开出来
  let pool = BOX_POOL[rarity].filter(x => cardAvailable(x[0]));
  if (!pool.length) pool = BOX_POOL.common.filter(x => cardAvailable(x[0]));
  if (!pool.length) pool = BOX_POOL.common;
...
function place(type, row, col, modules) {
  ...
  if (type === 'lilypad') {
    // 平台不是机器：它只是把这一格变成能站人的地方
    pads[row][col] = true;
    spawnParts(cellCx(col), cellCy(row), '#6fd8ff', 10, 90, 0.4, 'spark');
    sfx('place');
    return true;
  }
...
function snapshotPads() { return pads.map(row => row.slice()); }
// keepEnergy：撤销时不退能量（盲盒用——退了就能无限重摇）
function pushUndo(label, keepEnergy) {
  undoStack.push({ label, cells: snapshotBoard(), pads: snapshotPads(),
    energy, cd: { ...classicCd }, keepEnergy: !!keepEnergy });
...
function undoLast() {
  ...
  if (u.pads) pads = u.pads.map(row => row.slice());
...
function pickRow(type) {
  ...
  const pool = okRows.length ? okRows : Array.from({ length: ROWS }, (_, i) => i);
...
function damageEnemy(e, d, kind, noFlash) {
  ...
  // 潜地中：在地面以下，谁也够不着
  if (e.under) return;
  // Boss 下潜突进的两秒：整个沉进水里，打不着
  if (e.diveT > 0) {
    if (Math.random() < 0.06) addFloat(e.x, rowCy(e) - 40, '下潜中', '#9fd8ff');
    return;
  }
  // 隐匿状态免疫远程攻击
      ...
      for (const mod of m.modules) {
        ...
        if (kind === 'shot') {
          ...
          if ((m.mcd.rocket || 0) <= 0) {
            if (enemyAhead(r, cx)) {
              m.mcd.rocket = st('cd');
              // 鱼雷艇坞：贴水面走的鱼雷，对会游、会飞的都加倍
              bullets.push({ kind: 'rocket', row: r, x: cx + 20, dmg: 900 * lv, speed: 430,
                hit: new Set(), torp: m.type === 'torpedo' });
          ...
          // 黑洞：吸住 + 定身，并按「当前生命的百分比」撕一刀（Boss 减半，不然直接秒）
          if ((m.mcd.blackhole || 0) <= 0) {
            const rad = CELL_W * st('radius');
            const my = cellCy(r);
            const hits = enemies.filter(e => !e.dead && !e.under
              && Math.hypot(e.x - cx, rowCy(e) - my) < rad);
            if (hits.length) {
              m.mcd.blackhole = st('cd');
              m.pulse = 0.8;
              const pct = st('pct');
              for (const e of hits) {
                e.stunT = Math.max(e.stunT, st('hold') * (e.boss ? 0.5 : 1));
                e.pulled = 0.6;
                if (!e.boss) e.x = Math.max(cx + 14, e.x - 34);
                damageEnemy(e, e.hp * pct * (e.boss ? 0.5 : 1), 'true');
                spawnParts(e.x, rowCy(e), '#c98aff', 6, 90, 0.5, 'spark');
              }
...
// 潜艇是沉在水下的：敌人的炮弹从它头上飞过去
function submerged(m) { return !!(m && m.modules && hasKind(m, 'sub') && terrainAt(m.row, m.col) === 'W'); }
...
function updateEnemyBullets(dt) {
  for (let i = ebullets.length - 1; i >= 0; i--) {
    ...
    if (col >= 0 && col < COLS) {
      ...
      if (o && !isWalkable(o) && !submerged(o) && Math.abs(b.x - cellCx(col)) < CELL_W * 0.46) hit = { o, col };
    }
    if (hit) {
  ...
  for (let i = shells.length - 1; i >= 0; i--) {
    ...
    if (sh.t >= sh.dur) {
      ...
      for (const e of enemies) {
        ...
        if (sh.shatter) addShatter(e, sh.shatter);
        damageEnemy(e, sh.dmg * (d < sh.splash * 0.5 ? 1 : 0.6), 'ranged');
      }
...
function updatePools(dt) {
  for (let i = pools.length - 1; i >= 0; i--) {
    ...
    if (pl.own) {
      // 岩浆喷口烧出来的是「我方」岩浆：烧敌人，不烧自己的机器
      for (const e of enemies) {
        if (e.dead || e.under || e.fly || e.row !== pl.row) continue;
        if (Math.abs(e.x - pl.x) > CELL_W * 0.6) continue;
        damageEnemy(e, pl.dps * dt, 'true', true);
        e.burnT = Math.max(e.burnT, 1.2);
        e.burnDps = Math.max(e.burnDps, pl.dps * 0.35);
      }
    } else {
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    if (b.pierce) {
      for (const e of enemies) {
        ...
        damageEnemy(e, b.dmg * (b.torp && (e.swim || e.fly) ? 2 : 1), 'ranged');
    ...
    if (hitEnemy) {
      ...
      if (b.burn) {
        hitEnemy.burnT = Math.max(hitEnemy.burnT, 3);
        hitEnemy.burnDps = Math.max(hitEnemy.burnDps, b.burn);
        spawnParts(b.x, rowCy(hitEnemy), '#ff9d2e', 4, 70, 0.35, 'spark');
      }
      if (b.kind === 'ice') {
        hitEnemy.slowT = 3;
...
function updateEnemies(dt) {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    // 关底 Boss 掉到 rage 血线以下就进狂暴：提速、召唤加快、大招变频
    if (e.rage && !e.raging && e.hp <= e.maxHp * e.rage) {
      e.raging = true;
      e.speed *= 1.35;
      e.summonCd *= 0.55; e.summonT = Math.min(e.summonT, e.summonCd);
      e.skillCd *= 0.6;   e.skillT = Math.min(e.skillT, e.skillCd);
      addFloat(e.x, rowCy(e) - 92, '【狂暴】' + ENEMIES[e.type].name, '#ff3b3b');
      spawnParts(e.x, rowCy(e), '#ff3b3b', 30, 200, 0.9, 'spark');
      shake(0.5, 9);
      sfx('horn');
    }
...
/* ===== 关底 Boss 的专属大招 =====
   共用一套节奏：skillT 到点 → 起手 castT（给玩家一个能看见的预兆）→ 落地。
   每招都打「机器」而不是只掉血，这样玩家必须重建防线，而不是站着对拼 DPS。 */
function bossSkill(e) {
  const gy = rowCy(e);
  const K = e.skill;
  if (K === 'slam') {
    // 震地：脚下两格内的机器全部瘫痪并受创
    const rad = CELL_W * 2.2;
    shocks.push({ x: e.x, y: gy + 10, t: 0.6, max: 0.6, reach: rad, color: '#ff9d2e' });
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
      const m = grid[r][c];
      if (!m) continue;
      if (Math.hypot(cellCx(c) - e.x, cellCy(r) - gy) > rad) continue;
      m.stunT = Math.max(m.stunT || 0, 2.2);
      damageMachine(m, e.dmg * 0.55);
    }
    addFloat(e.x, gy - 92, '震地！', '#ff9d2e');
    shake(0.55, 11); sfx('boom');
  } else if (K === 'eruption') {
    // 喷发：本行前方连着烧出三摊岩浆
    for (let k = 0; k < 3; k++) {
      pools.push({ x: e.x - 70 - k * 78, row: e.row, t: 7, max: 7, dps: 60 + wave * 3 });
    }
    spawnParts(e.x, gy, '#ff7a2e', 26, 190, 0.8, 'spark');
    addFloat(e.x, gy - 92, '岩浆喷发！', '#ff7a2e');
    shake(0.5, 10); sfx('boom');
  } else if (K === 'blizzard') {
    // 暴雪：全场机器冻住，打不出东西
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
      const m = grid[r][c];
      if (m) m.stunT = Math.max(m.stunT || 0, 2.6);
    }
    for (let i = 0; i < 26; i++) {
      spawnParts(rand(GRID_X, FIELD_X), rand(GRID_Y, GRID_Y + ROWS * CELL_H), '#bfe9ff', 2, 60, 1.1, 'spark');
    }
    addFloat(e.x, gy - 92, '暴风雪！全场冻结', '#bfe9ff');
    shake(0.4, 7); sfx('ice');
  } else if (K === 'barrage') {
    // 弹幕：五行同时各来一发榴弹
    for (let r = 0; r < ROWS; r++) {
      ebullets.push({ row: r, x: e.x - 20, y: cellCy(r) - 6,
        speed: 260, dmg: e.rdmg || 110, kind: 'grenade', spin: 0 });
    }
    addFloat(e.x, gy - 92, '全行齐射！', '#c98aff');
    shake(0.35, 7); sfx('shoot');
  } else if (K === 'dive') {
    // 下潜：沉下去两秒无敌，再从驻守区的另一条通道冒出来——逼你把防线铺开
    e.diveT = 2;
    spawnParts(e.x, gy, '#9fd8ff', 22, 170, 0.8, 'spark');
    const info = ENEMIES[e.type];
    const rows = [];
    for (let r = 0; r < ROWS; r++) if (r !== e.row && rowPassable(info, r)) rows.push(r);
    if (rows.length) e.row = rows[Math.floor(Math.random() * rows.length)];
    e.x = kingHoldX() + rand(0, CELL_W * 1.6);
    addFloat(e.x, rowCy(e) - 92, '下潜换道！', '#9fd8ff');
    sfx('ice');
  } else if (K === 'siphon') {
    ...
    if (take > 0) {
      ...
      for (let k = 0; k < n; k++) {
        ...
        spawnEnemy('sentinel', r2);
        const ne = enemies[enemies.length - 1];
        ne.x = clamp(e.x - CELL_W * (0.6 + k * 0.5), GRID_X + 60, FIELD_X);
        spawnParts(ne.x, rowCy(ne), '#ffc531', 14, 130, 0.6, 'spark');
      }
      addFloat(e.x, gy - 74, '变出 黄金石像 ×' + n, '#ffc531');
    } else {
    ...
    for (const [r, c] of all.slice(0, 3)) {
      ...
      if (!m) continue;
      m.sh = 0;
      damageMachine(m, e.dmg * 0.8);
      shocks.push({ x: cellCx(c), y: cellCy(r), t: 0.5, max: 0.5, reach: 40, color: '#c98aff', suck: true });
    }
    // 闪现也只在驻守区里换位置
    const rows2 = [];
    for (let r = 0; r < ROWS; r++) if (rowPassable(ENEMIES[e.type], r)) rows2.push(r);
    if (rows2.length) e.row = rows2[Math.floor(Math.random() * rows2.length)];
    e.x = kingHoldX() + rand(0, CELL_W * 1.8);
    addFloat(e.x, rowCy(e) - 92, '虚空裂隙！', '#c98aff');
...
function updateAdvance(dt) {
  ...
  advanceT -= dt;
  const btn = $('nextStageBtn');
  if (btn) btn.textContent = '▶ 下一关（' + Math.max(1, Math.ceil(advanceT)) + '）';
  if (advanceT <= 0) goNextStage();
}
...
cv.addEventListener('pointerdown', ev => {
  ...
  if (sel && sel.mode === 'shovel') {
    if (grid[cell.r][cell.c]) {
      ...
      renderTray();
    } else if (hasPad(cell.r, cell.c)) {
      // 格子上没机器了，再点一下才拆平台——不会误伤
      pushUndo('拆除 水上平台');
      pads[cell.r][cell.c] = false;
      spawnParts(cellCx(cell.c), cellCy(cell.r), '#6fd8ff', 8, 80, 0.4, 'spark');
      sfx('break');
      renderTray();
    }
    return;
...
function renderStageSel() {
  ...
  styleRow.className = 'styleRow';
  for (const k of ['box', 'classic', 'creative']) {
    const sb = document.createElement('button');
    sb.className = 'styleBtn' + (campStyle === k ? ' on' : '');
    sb.innerHTML = '<b>' + STYLE_NAME[k] + '</b>' + STYLE_TIP[k];
    sb.addEventListener('click', () => { campStyle = k; saveStyle(); renderStageSel(); });
    styleRow.appendChild(sb);
  }
  box.appendChild(styleRow);
  // 第二步：这一关打哪个难度
  const diffRow = document.createElement('div');
  diffRow.className = 'styleRow diffRow';
  for (const k of ['easy', 'normal', 'hard']) {
    const db = document.createElement('button');
    db.className = 'styleBtn' + (campDiff === k ? ' on' : '');
    db.innerHTML = '<b>' + DIFFS[k].name + '</b>' + DIFFS[k].tip;
    db.addEventListener('click', () => { campDiff = k; saveDiff(); renderStageSel(); });
    diffRow.appendChild(db);
  }
  box.appendChild(diffRow);
  if (!allCardsUnlocked()) {
    const lk = document.createElement('div');
    lk.className = 'styleWarn';
    lk.textContent = '现在手上只有 ' + STARTER_CARDS.length + ' 台基础机器 —— 打通 '
      + MAPS[STARTER_MAP].name + ' 第 ' + STAGES_PER_MAP + ' 关，剩下的一次全解锁';
    box.appendChild(lk);
  }
  if (campStyle === 'creative') {
    const warn = document.createElement('div');
    warn.className = 'styleWarn';
    warn.textContent = '创造玩法是沙盒：可以在任何已解锁的关卡里随便实验，但打通了不算进度';
    box.appendChild(warn);
  }
  // 70 关不可能让人一格一格找，顶上先给一个「接着打」
  const far = furthestStage();
  const go = document.createElement('button');
  go.id = 'resumeCampaign';
  go.textContent = '▶ ' + STYLE_NAME[campStyle] + ' 接着打：' + MAPS[far.map].icon + ' ' + MAPS[far.map].name + ' 第 ' + far.st + ' 关';
  go.addEventListener('click', () => enterStage(far.map, far.st));
  box.appendChild(go);
  for (const mk of MAP_ORDER) {
    const M = MAPS[mk];
    const row = document.createElement('div');
    ...
    const done = [];
    for (let st = 1; st <= STAGES_PER_MAP; st++) if (pg[campaignId(mk, st)]) done.push(st);
    const h = document.createElement('h4');
    h.innerHTML = M.icon + ' ' + M.name +
      ' <small>' + done.length + '/' + STAGES_PER_MAP + ' · ' + M.tip + '</small>';
    row.appendChild(h);
    ...
    for (let st = 1; st <= STAGES_PER_MAP; st++) {
      ...
      const cleared = !!pg[campaignId(mk, st)];
      b.className = 'stageBtn' + (boss ? ' boss' : '') + (cleared ? ' done' : '');
      b.disabled = !open;
      b.innerHTML = boss
        ? '☠ 第' + st + '关<b>' + (open ? ENEMIES[M.boss].name : '未解锁') + '</b>'
        : (cleared ? '✓' : '') + st;
      b.title = boss ? '关底 Boss：' + ENEMIES[M.boss].name : M.name + ' 第 ' + st + ' 关';
...
function buildBackground() {
  ...
  // 地形层：按格子画。同一行里连着的同种地形合成一段，水面才不会一格一段地断开。
  for (let r = 0; r < ROWS; r++) {
    const y = GRID_Y + r * CELL_H;
    let c0 = 0;
    while (c0 < COLS) {
      const L = terrainRaw(r, c0);
      let c1 = c0;
      ...
      if (L === 'W' || L === 'L') {
        ...
        b.lineWidth = 2;
        b.beginPath();
        for (let x = x0; x <= x0 + wRun; x += 8) b.lineTo(x, y + 5 + Math.sin(x * 0.05 + r) * 2.4);
        b.stroke();
        if (L === 'L') {
          // 岩浆：翻上来的气泡 + 结壳的暗斑，别用水的焦散
          const nb = Math.max(4, Math.round(wRun / 46));
          for (let i = 0; i < nb; i++) {
            const wx = x0 + (i + 0.5) * (wRun / nb);
            const wy = y + 20 + ((i * 41) % (CELL_H - 34));
            b.fillStyle = i % 3 === 0 ? 'rgba(255,226,150,0.5)' : 'rgba(255,150,46,0.32)';
            b.beginPath(); b.arc(wx, wy, 2 + (i % 4), 0, TAU); b.fill();
          }
          for (let i = 0; i < Math.max(2, Math.round(wRun / 140)); i++) {
            const wx = x0 + 30 + ((i * 197) % Math.max(20, wRun - 60));
            const wy = y + 26 + ((i * 29) % Math.max(10, CELL_H - 50));
            b.fillStyle = 'rgba(58,18,10,0.55)';
            b.beginPath(); b.ellipse(wx, wy, 18, 7, 0, 0, TAU); b.fill();
          }
        } else {
          // 水下光斑
          b.strokeStyle = M.caustic || 'rgba(160,225,255,0.16)';
          b.lineWidth = 1.6;
          const nc = Math.max(3, Math.round(wRun / 55));
          for (let i = 0; i < nc; i++) {
            const wx = x0 + (i + 0.5) * (wRun / nc);
            const wy = y + 22 + ((i * 37) % (CELL_H - 36));
            b.beginPath();
            b.moveTo(wx - 12, wy); b.quadraticCurveTo(wx, wy - 6, wx + 12, wy);
            b.stroke();
          }
        }
        // 竖水道的两侧岸线：让它看起来真的是切进沙滩的一条河
        if (laneOf(r) !== 'W') {
          const edge = M.foam || 'rgba(150,215,245,0.45)';
          b.strokeStyle = edge; b.lineWidth = 2;
          for (const ex of [x0, x0 + wRun]) {
            b.beginPath();
            for (let yy = y; yy <= y + CELL_H; yy += 8) b.lineTo(ex + Math.sin(yy * 0.06 + r) * 2.2, yy);
            b.stroke();
          }
        }
      } else if (L === 'S') {
        ...
        b.fillRect(x0, y, wRun, CELL_H);
        // 云：几坨圆叠成一团，飘在台子之间
        const ncl = Math.max(2, Math.round(wRun / 240));
        for (let i = 0; i < ncl; i++) {
          const cx2 = x0 + 46 + ((i * 233 + r * 151) % Math.max(20, wRun - 92));
  ...
  if (M.ember) {
    for (let i = 0; i < 54; i++) {
      const gx = GRID_X + ((i * 157) % (COLS * CELL_W));
      const gy = GRID_Y + ((i * 83) % (ROWS * CELL_H));
      b.fillStyle = i % 3 === 0 ? 'rgba(255,214,130,0.3)' : 'rgba(255,120,50,0.22)';
      b.beginPath(); b.arc(gx, gy, 0.9 + (i % 3) * 0.7, 0, TAU); b.fill();
    }
  }
  // 冰原：格子边缘积一层霜，地上撒些雪粒
  if (M.snow) {
    for (let r = 0; r < ROWS; r++) {
      const y = GRID_Y + r * CELL_H;
      for (let c = 0; c < COLS; c++) {
        if (terrainAt(r, c) !== 'G') continue;
        const x = GRID_X + c * CELL_W;
        const fg = b.createLinearGradient(0, y, 0, y + 16);
        fg.addColorStop(0, 'rgba(232,248,255,0.4)');
        fg.addColorStop(1, 'rgba(232,248,255,0)');
        b.fillStyle = fg;
        b.fillRect(x, y, CELL_W, 16);
      }
      for (let i = 0; i < 30; i++) {
        const gx = GRID_X + ((i * 173 + r * 61) % (COLS * CELL_W));
        if (terrainAt(r, colAtX(gx)) !== 'G') continue;
        const gy = y + ((i * 53) % CELL_H);
        b.fillStyle = 'rgba(242,252,255,0.35)';
        b.beginPath(); b.arc(gx, gy, 1 + (i % 2), 0, TAU); b.fill();
      }
    }
  }
    ...
    for (let r = 0; r < ROWS; r++) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (seaside) {
          ...
          // 沙漠：半埋的碎陶片
          b.beginPath();
          b.moveTo(gx - 6, gy + 3); b.lineTo(gx - 1, gy - 5); b.lineTo(gx + 6, gy + 3);
          b.stroke();
        }
      }
      if ((M.lanes[r + 1] || '') === 'W') {
...
function drawMachine(ctx, type, x, y, s, m) {
  if (type === 'lilypad') {           // 平台不是机器，只有卡面会走到这里
  ...
  const onWater = m && m.row !== undefined && terrainAt(m.row, m.col) === 'W' && !hasPad(m.row, m.col);
  if (onWater) {
  ...
  const classicPair = PAIR_TYPE.some(q => q[1] === type);   // 融合机型自带造型，不再染色也不挂副能力饰件
...
function drawChassis(ctx, type, pri, m) {
  switch (type) {
    ...
    case 'scrapult': return drawScrapult(ctx, m);
    case 'rivetgun': return drawRivetgun(ctx, m);
    case 'torpedo': return drawTorpedo(ctx, m);
    case 'conch': return drawConch(ctx, m);
    case 'moltengun': return drawMoltengun(ctx, m);
    case 'icespire': return drawIcespire(ctx, m);
    ...
    case 'brazier': return drawBrazier(ctx, m);
    case 'auroradome': return drawAuroradome(ctx, m);
    case 'skyhive': return drawSkyhive(ctx, m);
    case 'voideye': return drawVoideye(ctx, m);
  }
  if (!pri) return;
...
// 漩涡炮：一只横着的涡轮环，环里卷着水
// 水上平台：一块浮在水面的六边浮台，边上有系缆桩，水波一直在晃
function drawPad(ctx, x, y, r, c) {
  const bob = Math.sin(time * 1.4 + r * 1.3 + c * 0.7) * 1.8;
  ctx.save();
  ctx.translate(x, y + bob);
  // 水线波纹
  ctx.strokeStyle = 'rgba(186,236,255,0.32)';
  ctx.lineWidth = 1.6;
  ctx.beginPath(); ctx.ellipse(0, 26, 42, 10, 0, 0, TAU); ctx.stroke();
  // 台面
  const pts = [];
  for (let i = 0; i < 6; i++) {
    const a = i * TAU / 6 + Math.PI / 6;
    pts.push([Math.cos(a) * 38, Math.sin(a) * 15]);
  }
  ctx.fillStyle = '#2b3d4e';
  ctx.beginPath();
  pts.forEach(([px, py], i) => (i ? ctx.lineTo(px, py + 8) : ctx.moveTo(px, py + 8)));
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = cachedLG(ctx, 0, 10, 0, 26, [0, '#6f8ca4', 1, '#3c5467']);
  ctx.beginPath();
  pts.forEach(([px, py], i) => (i ? ctx.lineTo(px, py + 18) : ctx.moveTo(px, py + 18)));
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = 'rgba(200,232,255,0.4)';
  ctx.lineWidth = 1.4;
  ctx.beginPath();
  pts.forEach(([px, py], i) => (i ? ctx.lineTo(px, py + 18) : ctx.moveTo(px, py + 18)));
  ctx.closePath(); ctx.stroke();
  // 防滑条
  ctx.fillStyle = 'rgba(255,197,49,0.45)';
  for (let i = -1; i <= 1; i++) rr(ctx, i * 14 - 5, 14, 10, 3, 1.5), ctx.fill();
  // 四角的系缆桩
  ctx.fillStyle = '#8aa2b8';
  for (const sx of [-30, 30]) { rr(ctx, sx - 3, 8, 6, 9, 2); ctx.fill(); }
  ctx.restore();
}

function drawVortex(ctx, m, lv) {
  const P = themed(pal(lv));
  const sp = time * (lv >= 3 ? 4.2 : 2.8);
  const pulse = m && m.pulse > 0 ? m.pulse / 0.5 : 0;
  pedestal(ctx, P, 44, lv);
  panel(ctx, -13, -8, 26, 26, 7, P);
  // 涡轮环：三圈同心，转速不一样
  for (let i = 0; i < 3; i++) {
    const rr2 = 12 + i * 6;
    ctx.save();
    ctx.translate(16, -12);
    ctx.rotate(sp * (i % 2 ? -0.6 : 0.9));
    ctx.strokeStyle = i === 1 ? P.trim : P.base;
    ctx.lineWidth = 3 - i * 0.5;
    ctx.beginPath(); ctx.ellipse(0, 0, rr2, rr2 * 0.42, 0, 0, TAU); ctx.stroke();
    ctx.restore();
  }
  // 环心的水涡
  emissive(ctx, 'rgba(111,216,255,' + (0.7 + pulse * 0.3) + ')', 14 + pulse * 10, () => {
    ctx.strokeStyle = '#6fd8ff';
    ctx.lineWidth = 2.2;
    for (let i = 0; i < 3; i++) {
      const a0 = sp * 1.6 + i * TAU / 3;
      ctx.beginPath();
      ctx.arc(16, -12, 5 + i * 3 + pulse * 3, a0, a0 + 2.1);
      ctx.stroke();
    }
  });
  // 侧面的进水口
  ctx.fillStyle = P.dark;
  rr(ctx, -20, -2, 10, 14, 3); ctx.fill();
  ctx.fillStyle = hexA('#6fd8ff', 0.7);
  rr(ctx, -18, 1, 6, 8, 2); ctx.fill();
}

// 流沙陷阱：一个斜着的漏斗，沙子一直往下漏
// 潜艇炮台：半沉在水下的艇身，只露出指挥塔和潜望镜
function drawSub(ctx, m, lv) {
  const P = themed(pal(lv));
  const t = time, bob = Math.sin(t * 1.5) * 2;
  const rec = (m && m.recoil > 0) ? m.recoil * 20 : 0;
  // 水面线以下的艇身（压暗）
  ctx.globalAlpha = 0.55;
  panel(ctx, -30, 14 + bob, 60, 18, 9, P);
  ctx.globalAlpha = 1;
  // 露出水面的部分
  panel(ctx, -26, -2 + bob, 52, 18, 9, P);
  // 指挥塔
  panel(ctx, -8, -20 + bob, 20, 20, 5, P);
  // 潜望镜
  ctx.fillStyle = P.dark;
  rr(ctx, 0, -34 + bob, 4, 16, 2); ctx.fill();
  rr(ctx, 0, -34 + bob, 11, 4, 2); ctx.fill();
  emissive(ctx, 'rgba(111,216,255,0.9)', 10, () => {
    ctx.fillStyle = '#6fd8ff';
    ctx.beginPath(); ctx.arc(10, -32 + bob, 2.2, 0, TAU); ctx.fill();
  });
  // 鱼雷管
  ctx.fillStyle = '#20262e';
  rr(ctx, 22 - rec, 2 + bob, 10, 9, 3); ctx.fill();
  if (lv >= 2) { ctx.fillStyle = '#20262e'; rr(ctx, 22 - rec, 12 + bob, 10, 7, 3); ctx.fill(); }
  // 舷窗
  ctx.fillStyle = hexA('#bfe9ff', 0.8);
  for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.arc(-16 + i * 11, 6 + bob, 2.4, 0, TAU); ctx.fill(); }
  hazard(ctx, -22, 9 + bob, 30, 5);
  // 尾流
  if (fxQuality > 0.4) for (let i = 0; i < 2; i++) {
    const ph = (t * 1.1 + i * 0.5) % 1;
    ctx.strokeStyle = 'rgba(186,236,255,' + (0.3 * (1 - ph)).toFixed(2) + ')';
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.ellipse(0, 24, 24 + ph * 18, 6 + ph * 4, 0, 0, TAU); ctx.stroke();
  }
}
...
function drawQuicksand(ctx, m, lv) {
  ...
  ctx.fillStyle = P.dark;
  ctx.beginPath();
  ctx.moveTo(-22, -22); ctx.lineTo(22, -22); ctx.lineTo(8, 8); ctx.lineTo(-8, 8);
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = P.base;
  ctx.beginPath();
  ctx.moveTo(-19, -19); ctx.lineTo(19, -19); ctx.lineTo(6, 5); ctx.lineTo(-6, 5);
  ctx.closePath(); ctx.fill();
  // 漏斗里的沙
  ctx.fillStyle = '#d8b378';
  ctx.beginPath();
  ctx.moveTo(-17, -12 + Math.sin(t * 1.4) * 1.5); ctx.lineTo(17, -12 + Math.sin(t * 1.4) * 1.5);
  ctx.lineTo(5, 4); ctx.lineTo(-5, 4);
  ctx.closePath(); ctx.fill();
  // 漏下来的沙柱
  ctx.fillStyle = 'rgba(216,179,120,0.85)';
  ctx.fillRect(-2.2, 6, 4.4, 14);
  for (let i = 0; i < 3; i++) {
    const ph = (t * 2.2 + i * 0.33) % 1;
    ctx.fillStyle = 'rgba(216,179,120,' + (0.7 * (1 - ph)).toFixed(2) + ')';
    ctx.beginPath(); ctx.arc(rand(-6, 6), 20 + ph * 10, 1.6, 0, TAU); ctx.fill();
  }
  // 沙堆
  ctx.fillStyle = '#c0995f';
  ctx.beginPath(); ctx.ellipse(0, 26, 20, 7, 0, Math.PI, TAU); ctx.fill();
  if (lv >= 2) {
    ctx.strokeStyle = P.trim; ctx.lineWidth = 2;
...
function drawBlackhole(ctx, m, lv) {
  const P = themed(pal(lv));
  const t = time, sp = t * 1.6;
  const pulse = m && m.pulse > 0 ? m.pulse / 0.8 : 0;
  pedestal(ctx, P, 46, lv);
  // 两根磁极臂
  for (const sgn of [-1, 1]) {
    ctx.fillStyle = P.dark;
    rr(ctx, sgn * 18 - 6, -26, 12, 34, 5); ctx.fill();
    ctx.fillStyle = P.trim;
    rr(ctx, sgn * 18 - 3, -23, 6, 10, 3); ctx.fill();
  }
  // 吸积盘
  ctx.save();
  ctx.translate(0, -16);
  for (let i = 0; i < 3; i++) {
    const a = sp + i * 0.7;
    ctx.strokeStyle = 'rgba(201,138,255,' + (0.55 - i * 0.13) + ')';
    ctx.lineWidth = 2.6 - i * 0.6;
    ctx.beginPath();
    ctx.ellipse(0, 0, 17 + i * 5 + pulse * 5, (5 + i * 2) * (0.6 + Math.abs(Math.cos(a)) * 0.5), a * 0.3, 0, TAU);
    ctx.stroke();
  }
  // 视界：中间彻底黑，边上一圈亮环
  emissive(ctx, 'rgba(201,138,255,' + (0.8 + pulse * 0.2) + ')', 18 + pulse * 12, () => {
    ctx.strokeStyle = '#e8d5ff';
    ctx.lineWidth = 2.4;
    ctx.beginPath(); ctx.arc(0, 0, 9 + pulse * 3, 0, TAU); ctx.stroke();
  });
  ctx.fillStyle = '#05030c';
  ctx.beginPath(); ctx.arc(0, 0, 8.5 + pulse * 3, 0, TAU); ctx.fill();
  // 被吸进去的碎屑
  if (fxQuality > 0.4) for (let i = 0; i < 4; i++) {
    const ph = 1 - ((t * 0.9 + i * 0.25) % 1);
    const a = sp * 2 + i * 1.57;
    ctx.fillStyle = 'rgba(201,138,255,' + (0.7 * ph).toFixed(2) + ')';
    ctx.beginPath();
    ctx.arc(Math.cos(a) * (10 + ph * 22), Math.sin(a) * (4 + ph * 9), 1.6, 0, TAU);
    ctx.fill();
  }
  ctx.restore();
}
...
// 废铁投石机：一条上了弦的抛臂，兜里装着绞碎的废铁
// 方尖碑炮：一根刻着符文的金碑，碑尖蓄光，碑身在发电
// 废铁电磁桩：一根缠满线圈的铁桩，桩头吸着一堆废铁片
function drawJunkcoil(ctx, m) {
  const P = pal(2), t = time;
  const pulse = m && m.pulse > 0 ? m.pulse / 0.6 : 0;
  pedestal(ctx, P, 44, 2);
  ctx.fillStyle = P.dark;
  rr(ctx, -9, -34, 18, 50, 5); ctx.fill();
  // 线圈
  for (let i = 0; i < 6; i++) {
    ctx.strokeStyle = i % 2 ? '#c08a3a' : '#e0a852';
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.ellipse(0, -28 + i * 8, 13, 4, 0, 0, TAU); ctx.stroke();
  }
  // 桩头的电磁铁
  ctx.fillStyle = '#4c5d70';
  rr(ctx, -16, -46, 32, 14, 4); ctx.fill();
  ctx.fillStyle = '#ff5d5d';
  rr(ctx, -14, -44, 13, 5, 2); ctx.fill();
  ctx.fillStyle = '#4cc2ff';
  rr(ctx, 1, -44, 13, 5, 2); ctx.fill();
  // 被吸住的废铁片
  ctx.fillStyle = '#8b8172';
  for (let i = 0; i < 4; i++) {
    const a = t * 1.4 + i * 1.57;
    ctx.save();
    ctx.translate(Math.cos(a) * (20 + pulse * 6), -46 + Math.sin(a) * 7);
    ctx.rotate(a);
    ctx.fillRect(-4, -2, 8, 4);
    ctx.restore();
  }
  emissive(ctx, 'rgba(127,215,255,' + (0.5 + pulse * 0.4) + ')', 12, () => {
    ctx.strokeStyle = '#9df0ff'; ctx.lineWidth = 1.8;
    ctx.beginPath(); ctx.ellipse(0, -46, 22 + pulse * 10, 8 + pulse * 4, 0, 0, TAU); ctx.stroke();
  });
}

// 结冰拖网：一台挂着冻硬渔网的绞盘车
function drawIcetrawler(ctx, m) {
  const P = pal(2), t = time;
  pedestal(ctx, P, 48, 2);
  panel(ctx, -20, -6, 40, 24, 6, P);
  // 绞盘
  ctx.save();
  ctx.translate(-8, 2);
  ctx.rotate(t * 1.1);
  ...
  ctx.restore();
  // 网架
  ctx.strokeStyle = '#8fa3b8'; ctx.lineWidth = 3;
  ctx.beginPath(); ctx.moveTo(10, -4); ctx.lineTo(30, -26); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(10, 10); ctx.lineTo(30, -6); ctx.stroke();
  // 冻硬的网
  ctx.strokeStyle = 'rgba(200,238,255,0.8)'; ctx.lineWidth = 1.4;
  for (let i = 0; i <= 4; i++) {
    ctx.beginPath(); ctx.moveTo(30 - i * 5, -26 + i * 5); ctx.lineTo(30 - i * 5 + 4, -4 + i * 2); ctx.stroke();
  }
  for (let i = 0; i <= 3; i++) {
    ctx.beginPath(); ctx.moveTo(12 + i * 5, -8 + i * 2); ctx.lineTo(30 - i * 2, -22 + i * 6); ctx.stroke();
  }
  // 网上挂的冰碴
  ctx.fillStyle = 'rgba(232,250,255,0.9)';
  for (const [dx, dy] of [[20, -14], [26, -6], [14, -2]]) {
    ctx.beginPath();
    ctx.moveTo(dx - 2, dy); ctx.lineTo(dx, dy + 7 + Math.sin(t * 2 + dx) * 1.2); ctx.lineTo(dx + 2, dy);
    ctx.closePath(); ctx.fill();
  }
}

// 浊浪号角：一只斜插在架子上的绿铜号角，号口吐着毒雾
function drawTidehorn(ctx, m) {
  const P = pal(2), t = time;
  pedestal(ctx, P, 46, 2);
  ctx.fillStyle = P.dark;
  rr(ctx, -6, -6, 12, 24, 4); ctx.fill();
  // 号角
  ctx.save();
  ctx.translate(-4, -12);
  ctx.rotate(-0.22);
  ctx.fillStyle = '#4e8a5c';
  ctx.beginPath();
  ctx.moveTo(6, -6); ctx.lineTo(-30, -18); ctx.lineTo(-30, 14); ctx.lineTo(6, 6);
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = '#6fb37e';
  ctx.beginPath();
  ctx.moveTo(4, -4); ctx.lineTo(-26, -14); ctx.lineTo(-26, 10); ctx.lineTo(4, 4);
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = '#2f5c3c';
  ctx.beginPath(); ctx.ellipse(-29, -2, 3.5, 15, 0, 0, TAU); ctx.fill();
  ctx.restore();
  // 吹出去的毒浪
  for (let i = 0; i < 3; i++) {
    const ph = (t * 1.1 + i * 0.34) % 1;
    ctx.strokeStyle = 'rgba(139,224,74,' + (0.6 * (1 - ph)).toFixed(2) + ')';
    ctx.lineWidth = 2.4;
    ctx.beginPath();
    ctx.arc(-34, -14, 6 + ph * 22, Math.PI * 0.68, Math.PI * 1.32);
    ctx.stroke();
  }
  // 号身上的铜环
  ctx.strokeStyle = '#ffc531'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-2, -16); ctx.lineTo(-2, 2); ctx.stroke();
}

// 炙铁火盆：一张烧红的铁栅，栅上支着尖刺
function drawBrazier(ctx, m) {
  const P = pal(2), t = time, glow = 0.55 + Math.sin(t * 3.2) * 0.3;
  pedestal(ctx, P, 52, 2);
  // 盆
  ctx.fillStyle = '#3a2a1e';
  ctx.beginPath();
  ctx.moveTo(-26, -6); ctx.lineTo(26, -6); ctx.lineTo(20, 18); ctx.lineTo(-20, 18);
  ctx.closePath(); ctx.fill();
  // 炭火
  emissive(ctx, 'rgba(255,140,50,' + glow + ')', 16, () => {
    ctx.fillStyle = cachedRG(ctx, 0, -4, 1, 0, -4, 24, [0, '#fff0b0', 0.45, '#ff7a2e', 1, '#8c1f04']);
    ctx.beginPath(); ctx.ellipse(0, -4, 24, 7, 0, 0, TAU); ctx.fill();
  });
  // 栅条
  ctx.fillStyle = '#5c4234';
  for (let i = -3; i <= 3; i++) rr(ctx, i * 7 - 1.6, -8, 3.2, 6, 1), ctx.fill();
  // 尖刺
  ctx.fillStyle = '#c8d4e0';
  for (let i = -2; i <= 2; i++) {
    const hgt = 16 - Math.abs(i) * 2;
    ctx.beginPath();
    ctx.moveTo(i * 10 - 4, -8); ctx.lineTo(i * 10, -8 - hgt); ctx.lineTo(i * 10 + 4, -8);
    ctx.closePath(); ctx.fill();
    // 尖上烧红的一截
    ctx.fillStyle = hexA('#ff7a2e', 0.8);
    ctx.beginPath();
    ctx.moveTo(i * 10 - 2, -8 - hgt * 0.5); ctx.lineTo(i * 10, -8 - hgt); ctx.lineTo(i * 10 + 2, -8 - hgt * 0.5);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#c8d4e0';
  }
  if (fxQuality > 0.4) for (let i = 0; i < 3; i++) {
    const ph = (t * 1.4 + i * 0.33) % 1;
    ctx.fillStyle = 'rgba(255,180,74,' + (0.6 * (1 - ph)).toFixed(2) + ')';
    ctx.beginPath(); ctx.arc(rand(-16, 16), -10 - ph * 26, 2 * (1 - ph * 0.5), 0, TAU); ctx.fill();
  }
}

// 极光护穹：一只半透明的穹顶，穹里挂着脉冲环
function drawAuroradome(ctx, m) {
  const P = pal(3), t = time;
  pedestal(ctx, P, 46, 3);
  panel(ctx, -16, 2, 32, 16, 5, P);
  // 三根支柱
  ctx.fillStyle = P.dark;
  for (const sx of [-18, 0, 18]) { rr(ctx, sx - 2.5, -14, 5, 18, 2); ctx.fill(); }
  // 穹顶
  const grd = ctx.createLinearGradient(0, -44, 0, -6);
  grd.addColorStop(0, 'rgba(160,255,214,0.45)');
  grd.addColorStop(0.5, 'rgba(120,200,255,0.3)');
  grd.addColorStop(1, 'rgba(120,200,255,0.05)');
  ctx.fillStyle = grd;
  ctx.beginPath(); ctx.arc(0, -12, 26, Math.PI, TAU); ctx.fill();
  ctx.strokeStyle = 'rgba(180,255,230,0.75)'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.arc(0, -12, 26, Math.PI, TAU); ctx.stroke();
  // 穹里流动的极光带
  for (let i = 0; i < 3; i++) {
    const ph = (t * 0.5 + i * 0.33) % 1;
    ctx.strokeStyle = 'rgba(' + (i % 2 ? '160,255,214' : '150,190,255') + ',' + (0.5 * (1 - Math.abs(ph - 0.5) * 2)).toFixed(2) + ')';
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(0, -12, 8 + ph * 16, Math.PI * 1.1, Math.PI * 1.9);
    ctx.stroke();
  }
  // 脉冲环
  for (let i = 0; i < 2; i++) {
    const ph = (t * 1.1 + i * 0.5) % 1;
    ctx.strokeStyle = 'rgba(159,196,255,' + (0.55 * (1 - ph)).toFixed(2) + ')';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(0, 12, 18 + ph * 30, 6 + ph * 9, 0, 0, TAU); ctx.stroke();
  }
  emissive(ctx, 'rgba(160,255,214,0.9)', 12, () => {
    ctx.fillStyle = '#c8ffe8';
    ctx.beginPath(); ctx.arc(0, -14, 4, 0, TAU); ctx.fill();
  });
}

// 天穹蜂巢：六边巢体，巢口飞出小无人机，底下一圈加速环
function drawSkyhive(ctx, m) {
  const P = pal(3), t = time;
  pedestal(ctx, P, 44, 3);
  // 巢
  ctx.save();
  ctx.translate(0, -14);
  for (const [dx, dy, rr2] of [[0, 0, 17], [-14, 9, 11], [14, 9, 11]]) {
    ctx.fillStyle = dx === 0 ? '#e0b554' : '#c08a3a';
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
      const a = i * TAU / 6 + Math.PI / 6;
      const px = dx + Math.cos(a) * rr2, py = dy + Math.sin(a) * rr2;
      i ? ctx.lineTo(px, py) : ctx.moveTo(px, py);
    }
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#4a3410';
    ctx.beginPath(); ctx.arc(dx, dy, rr2 * 0.42, 0, TAU); ctx.fill();
  }
  ctx.restore();
  // 飞出去的小机
  for (let i = 0; i < 3; i++) {
    const a = t * 1.8 + i * 2.1;
    const px = Math.cos(a) * 30, py = -14 + Math.sin(a) * 14;
    ctx.fillStyle = '#9fb2c6';
    rr(ctx, px - 4, py - 2.5, 8, 5, 2); ctx.fill();
    ctx.strokeStyle = 'rgba(200,225,245,0.5)'; ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.moveTo(px - 6, py - 3); ctx.lineTo(px + 6, py - 3); ctx.stroke();
  }
  // 加速环
  for (let i = 0; i < 2; i++) {
    const ph = (t * 1.3 + i * 0.5) % 1;
    ctx.strokeStyle = 'rgba(255,215,100,' + (0.5 * (1 - ph)).toFixed(2) + ')';
    ctx.lineWidth = 2.2;
    ctx.beginPath(); ctx.ellipse(0, 14, 16 + ph * 26, 6 + ph * 8, 0, 0, TAU); ctx.stroke();
  }
}

// 虚空瞳：一只悬着的紫瞳，瞳前架着一根细长的狙击管
function drawVoideye(ctx, m) {
  const P = pal(3), t = time;
  const rec = (m && m.recoil > 0) ? m.recoil * 22 : 0;
  pedestal(ctx, P, 44, 3);
  panel(ctx, -14, 4, 28, 14, 5, P);
  // 悬浮的瞳
  const fy = -20 + Math.sin(t * 1.4) * 2.5;
  emissive(ctx, 'rgba(201,138,255,0.85)', 16, () => {
    ctx.fillStyle = cachedRG(ctx, 0, fy, 1, 0, fy, 22, [0, '#ffffff', 0.4, '#d8b8ff', 1, '#6b2ab0']);
    ctx.beginPath(); ctx.ellipse(0, fy, 21, 14, 0, 0, TAU); ctx.fill();
  });
  ctx.fillStyle = '#1a0a30';
  ctx.beginPath(); ctx.ellipse(-3 + Math.sin(t * 0.9) * 5, fy, 5.5, 10, 0, 0, TAU); ctx.fill();
  ctx.strokeStyle = 'rgba(232,213,255,0.85)'; ctx.lineWidth = 1.8;
  ctx.beginPath(); ctx.ellipse(0, fy, 21, 14, 0, 0, TAU); ctx.stroke();
  // 狙击管
  ctx.fillStyle = '#2f2545';
  rr(ctx, 6 - rec, fy - 3, 36, 7, 3); ctx.fill();
  ctx.fillStyle = '#a76ee8';
  rr(ctx, 14 - rec, fy - 1.5, 20, 3, 1.5); ctx.fill();
  // 前面的折射片
  ctx.strokeStyle = 'rgba(201,138,255,0.8)'; ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(30, fy - 16); ctx.lineTo(38, fy); ctx.lineTo(30, fy + 16);
  ctx.stroke();
  ctx.fillStyle = 'rgba(201,138,255,0.14)';
  ctx.beginPath();
  ctx.moveTo(30, fy - 16); ctx.lineTo(38, fy); ctx.lineTo(30, fy + 16);
  ctx.closePath(); ctx.fill();
}

function drawObeliskgun(ctx, m) {
  const t = time;
  const ch = m && m.charge !== undefined ? m.charge : 0.5;
  const P = pal(3);
  pedestal(ctx, P, 44, 3);
  // 碑身：上窄下宽
  const grd = ctx.createLinearGradient(-12, -46, 12, 18);
  grd.addColorStop(0, '#fbe7ad');
  grd.addColorStop(0.5, '#d8b44e');
  grd.addColorStop(1, '#8a6a1e');
  ctx.fillStyle = grd;
  ctx.beginPath();
  ctx.moveTo(-8, -44); ctx.lineTo(8, -44); ctx.lineTo(14, 18); ctx.lineTo(-14, 18);
  ctx.closePath(); ctx.fill();
  // 碑顶的金字塔尖
  ctx.fillStyle = '#ffdf8a';
  ctx.beginPath();
  ctx.moveTo(-9, -44); ctx.lineTo(0, -60); ctx.lineTo(9, -44);
  ctx.closePath(); ctx.fill();
  // 符文
  ctx.fillStyle = 'rgba(70,48,10,0.7)';
  for (let i = 0; i < 4; i++) {
    rr(ctx, -5, -34 + i * 12, 10, 3, 1.5); ctx.fill();
    ctx.beginPath(); ctx.arc(0, -28 + i * 12, 2, 0, TAU); ctx.fill();
  }
  // 蓄能：符文一格格亮起来
  emissive(ctx, 'rgba(255,197,49,' + (0.4 + ch * 0.55) + ')', 10 + ch * 12, () => {
    ctx.fillStyle = '#ffe8a8';
    const lit = Math.max(1, Math.round(ch * 4));
    for (let i = 0; i < lit; i++) { rr(ctx, -5, -34 + i * 12, 10, 3, 1.5); ctx.fill(); }
    ctx.beginPath(); ctx.arc(0, -52, 3 + ch * 3, 0, TAU); ctx.fill();
  });
  // 底座上跳出来的电荷（它在发电）
  if (fxQuality > 0.4) for (let i = 0; i < 2; i++) {
    const ph = (t * 1.2 + i * 0.5) % 1;
    ctx.fillStyle = 'rgba(255,197,49,' + (0.7 * (1 - ph)).toFixed(2) + ')';
    ctx.beginPath(); ctx.arc(-18 + i * 36, 12 - ph * 22, 2.4 * (1 - ph * 0.5), 0, TAU); ctx.fill();
  }
}
...
function drawSandworm(ctx, m) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let k = -1; k <= 1; k++) {
      ctx.beginPath();
      ctx.moveTo(k * 4 - 1.6, -2); ctx.lineTo(k * 4, -7); ctx.lineTo(k * 4 + 1.6, -2);
      ctx.closePath(); ctx.fill();
    }
    ctx.restore();
  }
  ...
  ctx.fillStyle = '#4c5d70';
  for (const sx of [-26, 26]) {
    ctx.beginPath(); ctx.arc(sx, 22, 4.5, Math.PI, TAU); ctx.fill();
    ctx.fillStyle = '#ff5d5d';
    ctx.beginPath(); ctx.arc(sx, 18, 1.6, 0, TAU); ctx.fill();
    ctx.fillStyle = '#4c5d70';
  }
}

function drawScrapult(ctx, m) {
  const P = pal(2);
  const rec = (m && m.recoil > 0) ? m.recoil * 2.6 : 0;
  pedestal(ctx, P, 48, 2);
  panel(ctx, -18, 0, 36, 18, 5, P);
  // 抛臂（后坐时压下去）
  ctx.save();
  ctx.translate(-6, -2);
  ctx.rotate(-0.85 + rec);
  ctx.fillStyle = P.dark;
  rr(ctx, -3, -34, 6, 36, 3); ctx.fill();
  // 兜里的废铁
  ctx.fillStyle = '#8b8172';
  ctx.beginPath(); ctx.arc(0, -34, 7, 0, TAU); ctx.fill();
  ctx.fillStyle = '#5f574c';
  ctx.fillRect(-4, -37, 3, 3);
  ctx.fillRect(1, -33, 4, 2.5);
  ctx.restore();
  // 绞盘
  ctx.fillStyle = P.base;
  ctx.beginPath(); ctx.arc(10, -2, 8, 0, TAU); ctx.fill();
  ctx.strokeStyle = P.trim; ctx.lineWidth = 1.8;
  for (let i = 0; i < 4; i++) {
    const a = time * 1.2 + i * TAU / 4;
    ctx.beginPath();
    ctx.moveTo(10, -2); ctx.lineTo(10 + Math.cos(a) * 7, -2 + Math.sin(a) * 7);
    ctx.stroke();
  }
  hazard(ctx, -18, 12, 36, 6);
}

// 铆钉机枪：一根短粗的钉枪，弹链上挂着钉子
function drawRivetgun(ctx, m) {
  const P = pal(2);
  const rec = (m && m.recoil > 0) ? m.recoil * 26 : 0;
  pedestal(ctx, P, 42, 2);
  panel(ctx, -14, 2, 28, 16, 5, P);
  ctx.fillStyle = P.dark;
  rr(ctx, 0 - rec, -14, 34, 14, 5); ctx.fill();
  ctx.fillStyle = '#20262e';
  rr(ctx, 30 - rec, -11, 10, 8, 3); ctx.fill();
  // 弹链：一排钉子
  ctx.fillStyle = '#c8d4e0';
  for (let i = 0; i < 5; i++) {
    const x = -16 + i * 7;
    ctx.beginPath();
    ctx.moveTo(x, 0); ctx.lineTo(x + 2.4, -6); ctx.lineTo(x + 4.8, 0);
    ctx.closePath(); ctx.fill();
  }
  // 机身上竖着的备用钉
  ...
  for (let i = 0; i < 3; i++) {
    ctx.beginPath();
    ctx.moveTo(-10 + i * 9, -16); ctx.lineTo(-7.5 + i * 9, -26); ctx.lineTo(-5 + i * 9, -16);
    ctx.closePath(); ctx.fill();
  }
}

// 鱼雷艇坞：贴着水面的发射槽，槽里躺着一枚鱼雷
function drawTorpedo(ctx, m) {
  const P = pal(2);
  const rec = (m && m.recoil > 0) ? m.recoil * 26 : 0;
  pedestal(ctx, P, 50, 2);
  // 船坞
  ctx.fillStyle = P.dark;
  ctx.beginPath();
  ctx.moveTo(-24, 16); ctx.lineTo(24, 16); ctx.lineTo(19, -4); ctx.lineTo(-19, -4);
  ctx.closePath(); ctx.fill();
  panel(ctx, -20, -10, 40, 12, 4, P);
  // 鱼雷
  ctx.fillStyle = '#3d4a58';
  rr(ctx, -16 - rec, -20, 34, 12, 6); ctx.fill();
  ctx.fillStyle = '#8fb4c8';
  rr(ctx, -14 - rec, -18, 20, 5, 2.5); ctx.fill();
  ctx.fillStyle = '#ff5d5d';
  ctx.beginPath();
  ctx.arc(-16 - rec, -14, 5, Math.PI / 2, Math.PI * 1.5);
  ctx.fill();
  // 尾鳍
  ctx.fillStyle = '#5c7182';
  ctx.beginPath();
  ctx.moveTo(18 - rec, -20); ctx.lineTo(26 - rec, -25); ctx.lineTo(26 - rec, -3); ctx.lineTo(18 - rec, -8);
  ctx.closePath(); ctx.fill();
  // 水花
  ctx.strokeStyle = 'rgba(111,216,255,0.5)'; ctx.lineWidth = 1.6;
  ctx.beginPath(); ctx.ellipse(0, 20, 22, 5, 0, 0, TAU); ctx.stroke();
}

// 号角贝垒：一只立起来的大贝壳，壳口是个喇叭
function drawConch(ctx, m) {
  const P = pal(2);
  const t = time;
  pedestal(ctx, P, 54, 2);
  // 壳：一圈圈收窄
  for (let i = 0; i < 4; i++) {
    const w = 46 - i * 9, y = 16 - i * 11;
    ctx.fillStyle = i % 2 ? '#d99a6c' : '#c4834f';
    ctx.beginPath();
    ctx.ellipse(0, y, w / 2, 8 - i * 0.8, 0, Math.PI, TAU);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,240,215,0.35)';
    ctx.fillRect(-w / 2 + 3, y - 7, w - 6, 1.6);
  }
  // 喇叭口
  ctx.fillStyle = '#e8bb8f';
  ctx.beginPath();
  ctx.moveTo(-26, -6); ctx.lineTo(-44, -16); ctx.lineTo(-44, 10); ctx.lineTo(-26, 6);
  ctx.closePath(); ctx.fill();
  ctx.fillStyle = '#9c5f38';
  ctx.beginPath(); ctx.ellipse(-43, -3, 3.5, 12, 0, 0, TAU); ctx.fill();
  // 吹出来的声波
  ctx.strokeStyle = 'rgba(255,222,180,0.55)'; ctx.lineWidth = 2;
  for (let i = 0; i < 2; i++) {
    const ph = (t * 1.2 + i * 0.5) % 1;
    ctx.globalAlpha = 0.6 * (1 - ph);
    ctx.beginPath();
    ctx.arc(-44, -3, 6 + ph * 18, Math.PI * 0.72, Math.PI * 1.28);
    ctx.stroke();
  }
  ctx.globalAlpha = 1;
}

// 熔铁机炮：炮管烧得通红，弹匣是一口小熔炉
function drawMoltengun(ctx, m) {
  const P = pal(2);
  const rec = (m && m.recoil > 0) ? m.recoil * 26 : 0;
  const t = time, glow = 0.6 + Math.sin(t * 3.4) * 0.28;
  pedestal(ctx, P, 44, 2);
  panel(ctx, -15, 2, 30, 16, 5, P);
  // 熔炉弹匣
  ctx.fillStyle = '#4a2e1e';
  rr(ctx, -14, -18, 18, 20, 5); ctx.fill();
  emissive(ctx, 'rgba(255,140,50,' + glow + ')', 13, () => {
    ctx.fillStyle = cachedRG(ctx, -5, -8, 1, -5, -8, 9, [0, '#fff0b0', 0.5, '#ff9d2e', 1, '#c2350a']);
    ctx.beginPath(); ctx.arc(-5, -8, 6.5, 0, TAU); ctx.fill();
  });
  // 炮管：根部红、口部发亮
  ctx.fillStyle = '#5a3a26';
  rr(ctx, 2 - rec, -14, 32, 12, 5); ctx.fill();
  ctx.fillStyle = hexA('#ff7a2e', 0.75);
  rr(ctx, 6 - rec, -11, 20, 4, 2); ctx.fill();
  emissive(ctx, 'rgba(255,170,60,0.9)', 12, () => {
    ctx.fillStyle = '#ffc531';
    rr(ctx, 30 - rec, -12, 8, 8, 3); ctx.fill();
  });
  if (fxQuality > 0.4) for (let i = 0; i < 2; i++) {
    const ph = (t * 1.5 + i * 0.5) % 1;
    ctx.fillStyle = 'rgba(255,180,74,' + (0.6 * (1 - ph)).toFixed(2) + ')';
    ctx.beginPath(); ctx.arc(-5 + rand(-4, 4), -20 - ph * 18, 2, 0, TAU); ctx.fill();
  }
}

// 冰晶尖塔：一簇从地里长出来的冰刺，塔顶最高
function drawIcespire(ctx, m) {
  const P = pal(2);
  const t = time;
  pedestal(ctx, P, 48, 2);
  // 地上的冰刺地
  ctx.fillStyle = 'rgba(160,210,236,0.7)';
  for (let i = -3; i <= 3; i++) {
    const h = 8 + Math.abs(3 - Math.abs(i)) * 3;
    ctx.beginPath();
    ctx.moveTo(i * 7 - 3, 18); ctx.lineTo(i * 7, 18 - h); ctx.lineTo(i * 7 + 3, 18);
    ctx.closePath(); ctx.fill();
  }
  // 主塔：三根高矮不一的冰柱
  for (const [dx, h, w] of [[-11, 30, 9], [2, 46, 12], [13, 26, 8]]) {
    ctx.fillStyle = 'rgba(186,226,246,0.9)';
    ctx.beginPath();
    ctx.moveTo(dx - w / 2, 14); ctx.lineTo(dx, 14 - h); ctx.lineTo(dx + w / 2, 14);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = 'rgba(240,252,255,0.55)';
    ctx.beginPath();
    ctx.moveTo(dx - w / 2 + 2, 13); ctx.lineTo(dx - 1, 16 - h); ctx.lineTo(dx, 13);
    ctx.closePath(); ctx.fill();
  }
  // 塔尖的寒光
  emissive(ctx, 'rgba(191,233,255,' + (0.6 + Math.sin(t * 2.6) * 0.3) + ')', 14, () => {
    ctx.fillStyle = '#e8faff';
    ctx.beginPath(); ctx.arc(2, -32, 3.4, 0, TAU); ctx.fill();
  });
}

// 天穹长枪：一根斜指天空的激光矛
function drawSkylance(ctx, m) {
  const P = pal(3);
  const t = time;
  const rec = (m && m.recoil > 0) ? m.recoil * 18 : 0;
  pedestal(ctx, P, 44, 3);
  panel(ctx, -13, 0, 26, 18, 5, P);
  // 万向座
  ctx.fillStyle = P.dark;
  ...
  ctx.rotate(-0.62);
  ctx.fillStyle = P.base;
  rr(ctx, -6 + rec, -6, 46, 12, 5); ctx.fill();
  ctx.fillStyle = P.trim;
  rr(ctx, 4 + rec, -4, 24, 4, 2); ctx.fill();
  // 枪口的聚焦环
  for (let i = 0; i < 2; i++) {
    ctx.strokeStyle = hexA('#7fd7ff', 0.8 - i * 0.25);
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(34 + rec + i * 6, 0, 3, 8 - i * 1.6, 0, 0, TAU); ctx.stroke();
  }
  emissive(ctx, 'rgba(127,215,255,' + (0.6 + Math.sin(t * 5) * 0.3) + ')', 14, () => {
    ctx.fillStyle = '#d8f4ff';
    ctx.beginPath(); ctx.arc(44 + rec, 0, 3.2, 0, TAU); ctx.fill();
  });
  ctx.restore();
  // 底座上的对空雷达
  ctx.strokeStyle = P.trim; ctx.lineWidth = 1.8;
  ctx.beginPath(); ctx.arc(-14, 4, 7, Math.PI, TAU); ctx.stroke();
}

// 虚空棱镜：悬空的紫水晶棱柱，底下压着一口小引力井
function drawVoidprism(ctx, m) {
  const P = pal(3);
  const t = time, sp = t * 1.3;
  const pulse = m && m.pulse > 0 ? m.pulse / 0.6 : 0;
  pedestal(ctx, P, 46, 3);
  // 底座的引力井
  ctx.save();
  ctx.translate(0, 8);
  for (let i = 0; i < 2; i++) {
    ctx.strokeStyle = 'rgba(201,138,255,' + (0.5 - i * 0.18) + ')';
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.ellipse(0, 0, 14 + i * 7 + pulse * 4, 5 + i * 2, 0, 0, TAU); ctx.stroke();
  }
  ctx.restore();
  // 悬空的棱柱
  const fy = -20 + Math.sin(t * 1.5) * 2.5;
  ctx.save();
  ctx.translate(0, fy);
  ctx.rotate(sp * 0.5);
  const grd = ctx.createLinearGradient(-12, -20, 12, 20);
  grd.addColorStop(0, '#e8d5ff');
  grd.addColorStop(0.5, '#a76ee8');
  grd.addColorStop(1, '#4a2080');
  ctx.fillStyle = grd;
  ctx.beginPath();
  ctx.moveTo(0, -22); ctx.lineTo(12, -6); ctx.lineTo(7, 20); ctx.lineTo(-7, 20); ctx.lineTo(-12, -6);
  ctx.closePath(); ctx.fill();
  ctx.strokeStyle = 'rgba(232,213,255,0.8)'; ctx.lineWidth = 1.6;
  ctx.beginPath(); ctx.moveTo(0, -22); ctx.lineTo(0, 20); ctx.stroke();
  ctx.restore();
  // 分出去的三束光
  emissive(ctx, 'rgba(201,138,255,0.9)', 12, () => {
    ctx.strokeStyle = 'rgba(232,213,255,0.75)';
    ctx.lineWidth = 2;
    for (const dy of [-16, 0, 16]) {
      ctx.beginPath();
      ctx.moveTo(10, fy); ctx.lineTo(30, fy + dy * 0.5);
      ctx.stroke();
    }
  });
}
...
function drawEnemy(e) {
  ...
  if (terrainAtX(e.row, e.x) === 'W' && !e.fly) {
    g.lineWidth = 1.6;
    for (let i = 0; i < 2; i++) {
  ...
  switch (e.type) {
    ...
    case 'slagcrawler': drawSlagCrawler(e, bob); break;
    case 'forgeknight': drawForgeKnight(e, bob); break;
    case 'glacier': drawGlacier(e, bob); break;
    case 'snowdrone': drawSnowDrone(e); break;
    case 'voidspawn': drawVoidSpawn(e, bob); break;
...
// 熔渣爬虫：泡在岩浆里的多节虫，节与节之间透出炉火
function drawSlagCrawler(e, bob) {
  const t = time;
  for (let i = 2; i >= 0; i--) {
    const x = 10 + i * 15, wob = Math.sin(t * 7 + i * 0.9 + e.anim) * 3;
    g.fillStyle = i % 2 ? '#5a2a18' : '#6e3520';
    g.beginPath(); g.ellipse(x, 2 + bob + wob, 11 - i, 9 - i * 0.8, 0, 0, TAU); g.fill();
    emissive(g, 'rgba(255,140,50,0.7)', 8, () => {
      g.fillStyle = '#ff9d2e';
      g.beginPath(); g.ellipse(x - 6, 2 + bob + wob, 2.6, 5, 0, 0, TAU); g.fill();
    });
  }
  eBody(-22, -12 + bob, 38, 24, 11, '#c9663a', '#7a3418', '#3d1a0c');
  // 壳缝里的熔流
  emissive(g, 'rgba(255,160,60,0.8)', 10, () => {
    g.fillStyle = '#ffb14a';
    rr(g, -16, -6 + bob, 26, 5, 2.5); g.fill();
  });
  // 口器
  g.fillStyle = '#8a3c1c';
  g.beginPath();
  g.moveTo(-22, -6 + bob); g.lineTo(-34, 2 + bob); g.lineTo(-22, 8 + bob);
  g.closePath(); g.fill();
  g.fillStyle = '#ffd98a';
  for (let i = 0; i < 3; i++) {
    g.beginPath();
    g.moveTo(-32 + i * 4, 0 + bob); g.lineTo(-30 + i * 4, 6 + bob); g.lineTo(-28 + i * 4, 0 + bob);
    g.closePath(); g.fill();
  }
  eEye(-14, -4 + bob, 3, '#ffe08a');
}

// 铸铁骑士：厚甲、塔盾、护心炉火
function drawForgeKnight(e, bob) {
  const t = time;
  g.fillStyle = '#3a2620';
  rr(g, -8, 18 + bob, 10, 14, 3); g.fill();
  rr(g, 4, 18 + bob, 10, 14, 3); g.fill();
  eBody(-16, -18 + bob, 34, 38, 9, '#9c8070', '#5a4034', '#2c1c16');
  hazardE(-13, -12 + bob, 28, 8);
  emissive(g, 'rgba(255,140,50,' + (0.6 + Math.sin(t * 3) * 0.2) + ')', 11, () => {
    g.fillStyle = '#ff9d2e';
    g.beginPath(); g.arc(1, 4 + bob, 6, 0, TAU); g.fill();
  });
  // 头盔 + 观察缝
  eBody(-11, -32 + bob, 24, 17, 6, '#ab8e7c', '#5f4436');
  g.fillStyle = '#ff5d5d';
  rr(g, -7, -26 + bob, 16, 4, 2); g.fill();
  // 塔盾（挡在身前）
  g.fillStyle = '#7a6252';
  rr(g, -34, -24 + bob, 16, 46, 5); g.fill();
  g.fillStyle = '#a89080';
  rr(g, -32, -21 + bob, 12, 40, 4); g.fill();
  g.strokeStyle = '#ffc531'; g.lineWidth = 2;
  g.beginPath(); g.moveTo(-26, -16 + bob); g.lineTo(-26, 14 + bob); g.stroke();
}

// 冰川推土机：一整块推着走的冰墙，底下是履带
function drawGlacier(e, bob) {
  const t = time;
  g.fillStyle = '#2a3e52';
  rr(g, -36, 16 + bob, 72, 15, 6); g.fill();
  g.fillStyle = '#42607e';
  for (let i = 0; i < 6; i++) rr(g, -33 + i * 12, 19 + bob, 8, 9, 2), g.fill();
  eBody(-34, -20 + bob, 66, 38, 8, '#8fb4ce', '#436a8c', '#1f3850');
  hazardE(-28, -14 + bob, 54, 9);
  // 前面推着的冰块
  g.fillStyle = 'rgba(206,240,255,0.85)';
  g.beginPath();
  g.moveTo(-34, -22 + bob); g.lineTo(-58, -2 + bob); g.lineTo(-52, 20 + bob); g.lineTo(-34, 20 + bob);
  g.closePath(); g.fill();
  g.strokeStyle = 'rgba(255,255,255,0.7)'; g.lineWidth = 1.6;
  g.beginPath(); g.moveTo(-52, -8 + bob); g.lineTo(-40, 6 + bob); g.stroke();
  eEye(-16, -6 + bob, 4, '#bfe9ff');
  eEye(2, -6 + bob, 4, '#bfe9ff');
  // 冒的寒气
  if (fxQuality > 0.5) for (let i = 0; i < 3; i++) {
    const ph = (t * 0.7 + i * 0.33) % 1;
    g.fillStyle = 'rgba(200,240,255,' + (0.26 * (1 - ph)).toFixed(3) + ')';
    g.beginPath(); g.arc(10 + ph * 8, -24 + bob - ph * 22, 4 + ph * 7, 0, TAU); g.fill();
  }
}

// 暴雪无人机：四叶旋翼裹着一团雪雾
function drawSnowDrone(e) {
  const t = time, spin = e.anim * 22;
  for (const [rx, ry] of [[-22, -8], [22, -8]]) eRotor(rx, ry, 13, spin, 'rgba(210,240,255,0.3)');
  eBody(-18, -8, 36, 20, 9, '#cfe6f5', '#5d84a0', '#27414f');
  hazardE(-13, -3, 26, 7);
  eEye(-7, -2, 3, '#7ad8ff');
  eEye(7, -2, 3, '#7ad8ff');
  // 腹下的冷凝喷口
  emissive(g, 'rgba(160,230,255,0.7)', 10, () => {
    g.fillStyle = '#bfe9ff';
    g.beginPath(); g.arc(0, 13, 5, 0, TAU); g.fill();
  });
  if (fxQuality > 0.5) for (let i = 0; i < 4; i++) {
    const ph = (t * 1.1 + i * 0.25) % 1;
    g.fillStyle = 'rgba(220,246,255,' + (0.3 * (1 - ph)).toFixed(3) + ')';
    g.beginPath(); g.arc(rand(-6, 6), 16 + ph * 22, 2.4 * (1 - ph * 0.5), 0, TAU); g.fill();
  }
}

// 虚空爬虫：半透明的紫色小体，身上飘着碎片
function drawVoidSpawn(e, bob) {
  const t = time;
  g.globalAlpha = 0.9;
  eBody(-16, -12 + bob, 32, 24, 12, '#b78aff', '#5f2fa0', '#2c1250');
  g.globalAlpha = 1;
  emissive(g, 'rgba(201,138,255,0.85)', 12, () => {
    g.fillStyle = '#e8d5ff';
    g.beginPath(); g.arc(0, 0 + bob, 5.5 + Math.sin(t * 5 + e.anim) * 1.2, 0, TAU); g.fill();
  });
  eEye(-7, -4 + bob, 2.8, '#ffffff');
  eEye(7, -4 + bob, 2.8, '#ffffff');
  // 环绕的碎片
  g.fillStyle = 'rgba(201,138,255,0.8)';
  for (let i = 0; i < 4; i++) {
    const a = t * 2.2 + i * 1.57;
    g.save();
    g.translate(Math.cos(a) * 22, bob + Math.sin(a) * 12);
    g.rotate(a);
...
// 裂隙行者：披着斗篷的瘦长体，身后拖着一道裂缝
function drawRiftWalker(e, bob) {
  const t = time;
  g.strokeStyle = 'rgba(201,138,255,0.55)'; g.lineWidth = 2;
  g.beginPath();
  g.moveTo(20, -18 + bob);
  g.lineTo(28 + Math.sin(t * 6) * 3, 0 + bob);
  g.lineTo(20, 18 + bob);
  g.stroke();
  g.fillStyle = '#3a2258';
  g.beginPath();
  g.moveTo(-14, -18 + bob); g.lineTo(14, -18 + bob); g.lineTo(18, 24 + bob); g.lineTo(-18, 24 + bob);
  g.closePath(); g.fill();
  eBody(-12, -26 + bob, 24, 20, 8, '#8a6ac0', '#452a70', '#1e1038');
  emissive(g, 'rgba(201,138,255,0.9)', 12, () => {
    g.fillStyle = '#e8d5ff';
    rr(g, -8, -20 + bob, 16, 4, 2); g.fill();
  });
  // 手里的裂隙刃
  g.strokeStyle = '#c98aff'; g.lineWidth = 3; g.lineCap = 'round';
  g.beginPath();
...
// 机械圣甲虫：低矮的金壳虫，鞘翅一张一合
function drawScarab(e, bob) {
  const t = time, flap = Math.sin(t * 9 + e.anim) * 0.35;
  // 腿
  g.strokeStyle = '#7a5a20'; g.lineWidth = 2.2; g.lineCap = 'round';
  for (let i = -1; i <= 1; i++) for (const sgn of [-1, 1]) {
    g.beginPath();
    g.moveTo(i * 7, 6 + bob);
    g.lineTo(i * 7 + sgn * 11, 16 + bob + Math.sin(t * 10 + i) * 2);
    g.stroke();
  }
  g.lineCap = 'butt';
  // 鞘翅
  for (const sgn of [-1, 1]) {
    ...
    g.translate(0, -4 + bob);
    g.rotate(sgn * flap);
    g.fillStyle = sgn < 0 ? '#e0b554' : '#c99a3e';
    g.beginPath(); g.ellipse(sgn * 8, 0, 14, 10, 0, 0, TAU); g.fill();
    g.restore();
  }
  eBody(-13, -10 + bob, 26, 17, 8, '#f0d488', '#a8801f', '#5e440d');
  // 头顶的圣甲纹
  g.fillStyle = '#4a3410';
  rr(g, -7, -7 + bob, 14, 3, 1.5); g.fill();
  eEye(-14, -4 + bob, 2.6, '#ff5d5d');
  eEye(-14, 3 + bob, 2.6, '#ff5d5d');
}

// 缠带机兵：裹着绷带的瘦高机体，绷带一直在飘
function drawMummyBot(e, bob) {
  const t = time;
  g.fillStyle = '#9c8a68';
  rr(g, -9, 14 + bob, 8, 14, 3); g.fill();
  rr(g, 2, 14 + bob, 8, 14, 3); g.fill();
  eBody(-15, -16 + bob, 30, 34, 8, '#d8c9a4', '#8d7b55', '#463c26');
  // 缠带
  g.strokeStyle = '#efe3c4'; g.lineWidth = 2.4;
  for (let i = 0; i < 4; i++) {
    const yy = -12 + i * 9 + bob;
    g.beginPath();
    g.moveTo(-15, yy); g.quadraticCurveTo(0, yy + 3, 15, yy - 1);
    g.stroke();
  }
  // 飘出来的一截
  g.strokeStyle = 'rgba(239,227,196,0.8)'; g.lineWidth = 2;
  g.beginPath();
  g.moveTo(15, -4 + bob);
  g.quadraticCurveTo(26 + Math.sin(t * 3) * 4, 2 + bob, 33, 12 + bob + Math.sin(t * 3.6) * 4);
  g.stroke();
  eBody(-11, -30 + bob, 22, 17, 6, '#e4d6b2', '#8d7b55');
  emissive(g, 'rgba(120,220,255,0.8)', 9, () => {
    g.fillStyle = '#8fe3ff';
    rr(g, -7, -25 + bob, 14, 4, 2); g.fill();
  });
}

// 黄金石像：法老用你的电变出来的护卫，通体金色、眼里有电
function drawSentinel(e, bob) {
  const t = time;
  g.fillStyle = '#8a6a1e';
  rr(g, -16, 16 + bob, 13, 16, 4); g.fill();
  rr(g, 3, 16 + bob, 13, 16, 4); g.fill();
  eBody(-21, -20 + bob, 42, 40, 8, '#f6dc94', '#b08a24', '#5c430c');
  hazardE(-17, -14 + bob, 34, 8);
  // 胸口的能量核：这就是你的电
  emissive(g, 'rgba(255,197,49,' + (0.6 + Math.sin(t * 3) * 0.28) + ')', 13, () => {
    g.fillStyle = cachedRG(g, 0, 4 + bob, 1, 0, 4 + bob, 11, [0, '#fff6cf', 0.5, '#ffc531', 1, '#a86f08']);
    g.beginPath(); g.arc(0, 4 + bob, 8, 0, TAU); g.fill();
  });
  // 法老头巾式的方头
  eBody(-15, -38 + bob, 30, 20, 5, '#fbe7ad', '#b08a24', '#5c430c');
  g.fillStyle = '#2d6fa8';
  rr(g, -15, -38 + bob, 30, 5, 2); g.fill();
  eEye(-7, -29 + bob, 3.2, '#8fe3ff');
  eEye(7, -29 + bob, 3.2, '#8fe3ff');
  // 交叉在胸前的两根权杖
  g.strokeStyle = '#d8b44e'; g.lineWidth = 3.4; g.lineCap = 'round';
  g.beginPath(); g.moveTo(-24, -6 + bob); g.lineTo(-14, 16 + bob); g.stroke();
  g.beginPath(); g.moveTo(24, -6 + bob); g.lineTo(14, 16 + bob); g.stroke();
  g.lineCap = 'butt';
}

// 机械法老王：一尊坐镇的金色石棺像，头巾、假胡子、交叉权杖，胸口是个电容库
function drawPharaoh(e) {
  const t = time, glow = 0.6 + Math.sin(t * 2.2) * 0.3;
  const casting = e.castT > 0;
  // 底座
  g.fillStyle = '#7a5a18';
  rr(g, -62, 24, 124, 22, 6); g.fill();
  g.fillStyle = '#96721f';
  rr(g, -56, 20, 112, 8, 4); g.fill();
  // 身体：梯形石棺
  ...
  g.fillStyle = grd;
  g.beginPath();
  g.moveTo(-40, -40); g.lineTo(40, -40); g.lineTo(52, 24); g.lineTo(-52, 24);
  g.closePath(); g.fill();
  hazardE(-38, -34, 76, 10);
  // 胸口的电容库：抽来的电堆在这里
  g.fillStyle = '#2a1d06';
  rr(g, -30, -14, 60, 30, 8); g.fill();
  emissive(g, 'rgba(255,197,49,' + glow + ')', casting ? 26 : 16, () => {
    g.fillStyle = cachedRG(g, 0, 1, 1, 0, 1, 28, [0, '#fff6cf', 0.45, '#ffc531', 1, '#8c5a04']);
    g.beginPath();
    g.ellipse(0, 1, 25 + (casting ? 3 : 0), 12 + Math.sin(t * 4) * 1.5, 0, 0, TAU);
    g.fill();
  });
  // 电容格栅
  g.fillStyle = 'rgba(40,26,6,0.8)';
  for (let i = -2; i <= 2; i++) g.fillRect(i * 11 - 2, -12, 4, 26);
  // 交叉的权杖与连枷
  g.strokeStyle = '#e8c96a'; g.lineWidth = 6; g.lineCap = 'round';
  g.beginPath(); g.moveTo(-46, -20); g.lineTo(-22, 14); g.stroke();
  g.beginPath(); g.moveTo(46, -20); g.lineTo(22, 14); g.stroke();
  g.lineCap = 'butt';
  g.fillStyle = '#2d6fa8';
  g.beginPath(); g.arc(-48, -24, 6, 0, TAU); g.fill();
  g.beginPath(); g.arc(48, -24, 6, 0, TAU); g.fill();
  // 头巾（ nemes ）：两片垂下来的金蓝条纹
  for (const sgn of [-1, 1]) {
    g.fillStyle = '#d8b44e';
    g.beginPath();
    g.moveTo(sgn * 20, -68); g.lineTo(sgn * 44, -52); g.lineTo(sgn * 38, -18); g.lineTo(sgn * 20, -30);
    g.closePath(); g.fill();
    g.fillStyle = 'rgba(45,111,168,0.75)';
    for (let i = 0; i < 3; i++) {
      g.beginPath();
      g.moveTo(sgn * (24 + i * 6), -62 + i * 4); g.lineTo(sgn * (42 - i * 2), -50 + i * 6);
      g.lineTo(sgn * (40 - i * 2), -44 + i * 6); g.lineTo(sgn * (24 + i * 6), -56 + i * 4);
      g.closePath(); g.fill();
    }
  }
  // 脸
  eBody(-22, -74, 44, 46, 10, '#f6dc94', '#b08a24', '#5c430c');
  emissive(g, 'rgba(143,227,255,0.9)', 12, () => {
    g.fillStyle = '#8fe3ff';
    rr(g, -15, -60, 12, 6, 3); g.fill();
    rr(g, 3, -60, 12, 6, 3); g.fill();
  });
  // 假胡子
  g.fillStyle = '#c9a43a';
  rr(g, -5, -32, 10, 16, 4); g.fill();
  // 额前的圣蛇
  g.fillStyle = '#ffc531';
  g.beginPath();
  g.moveTo(0, -74); g.lineTo(-6, -84); g.lineTo(0, -92); g.lineTo(6, -84);
  g.closePath(); g.fill();
  emissive(g, 'rgba(255,93,93,0.9)', 10, () => {
    g.fillStyle = '#ff5d5d';
    g.beginPath(); g.arc(0, -84, 2.6, 0, TAU); g.fill();
  });
  // 蓄力时：一圈往身上收的金色电流
  if (casting) {
    g.strokeStyle = 'rgba(255,197,49,0.8)'; g.lineWidth = 2;
    for (let i = 0; i < 4; i++) {
      const ph = 1 - ((t * 1.6 + i * 0.25) % 1);
      g.globalAlpha = 0.7 * (1 - ph);
      g.beginPath(); g.ellipse(0, -4, 30 + ph * 70, 16 + ph * 34, 0, 0, TAU); g.stroke();
    }
    g.globalAlpha = 1;
  }
}
...
// 熔炉魔像：胸口开着一扇炉门，浑身是流动的熔岩缝
function drawForgeGolem(e) {
  const t = time, glow = 0.6 + Math.sin(t * 2.6) * 0.3;
  const step = Math.sin(e.anim * 3.6);
  for (const sgn of [-1, 1]) {
    const lift = sgn > 0 ? Math.max(step, 0) * 5 : Math.max(-step, 0) * 5;
    g.fillStyle = '#3a2218';
    rr(g, sgn * 16 - 19, 18 - lift, 38, 28, 7); g.fill();
    g.fillStyle = '#50301f';
    rr(g, sgn * 16 - 23, 42 - lift, 46, 10, 4); g.fill();
  }
  // 两条熔岩臂
  for (const sgn of [-1, 1]) {
    const sw2 = Math.sin(e.anim * 3.6 + (sgn > 0 ? 1.6 : 0)) * 4;
    g.fillStyle = '#5a3320';
    rr(g, sgn * 54 - 10, -24 + sw2, 20, 44, 8); g.fill();
    emissive(g, 'rgba(255,140,50,' + glow + ')', 12, () => {
      g.fillStyle = '#ff9d2e';
      rr(g, sgn * 54 - 4, -18 + sw2, 8, 34, 4); g.fill();
    });
    eBody(sgn * 58 - 16, 16 + sw2, 32, 28, 8, '#b07048', '#6b3a1e', '#301608');
  }
  // 躯干：石壳
  eBody(-52, -36 + 0, 104, 66, 14, '#a66a44', '#63381d', '#2e1509');
  // 炉门
  g.fillStyle = '#1c0c06';
  rr(g, -30, -18, 60, 40, 8); g.fill();
  emissive(g, 'rgba(255,150,50,' + glow + ')', 20, () => {
    g.fillStyle = cachedRG(g, 0, 0, 1, 0, 0, 30, [0, '#fff0b0', 0.45, '#ff9d2e', 1, '#8c1f04']);
    g.save(); g.translate(0, 2);
    g.beginPath(); g.ellipse(0, 0, 26 + Math.sin(t * 3) * 2, 17, 0, 0, TAU); g.fill();
    g.restore();
  });
  // 炉栅
  g.fillStyle = 'rgba(30,12,6,0.8)';
  for (let i = -2; i <= 2; i++) g.fillRect(i * 11 - 2, -16, 4, 36);
  // 熔岩缝
  emissive(g, 'rgba(255,140,50,0.8)', 10, () => {
    g.fillStyle = '#ffb14a';
    rr(g, -46, -30, 30, 5, 2); g.fill();
    rr(g, 18, -30, 30, 5, 2); g.fill();
  });
  // 头：一块带火冠的石头
  eBody(-24, -66, 48, 32, 9, '#b57a50', '#68391c', '#2e1509');
  eEye(-11, -52, 4.6, '#ff9d2e');
  eEye(11, -52, 4.6, '#ff9d2e');
  emissive(g, 'rgba(255,170,60,0.9)', 14, () => {
    g.fillStyle = '#ffc531';
    for (let i = -2; i <= 2; i++) {
      const h = 16 + Math.sin(t * 6 + i) * 6;
      g.beginPath();
      g.moveTo(i * 11 - 5, -66); g.lineTo(i * 11, -66 - h); g.lineTo(i * 11 + 5, -66);
      g.closePath(); g.fill();
    }
  });
}

// 霜寒巨兽：四足冰甲兽，背上一排冰棱，嘴里吐寒气
function drawFrostBehemoth(e) {
  const t = time, step = Math.sin(e.anim * 3.2);
  g.fillStyle = '#2b4459';
  for (const sgn of [-1, 1]) for (const off of [-30, 22]) {
    const lift = (sgn > 0 ? Math.max(step, 0) : Math.max(-step, 0)) * 5;
    rr(g, off + sgn * 4 - 10, 18 - lift, 21, 28, 6); g.fill();
  }
  // 尾
  g.strokeStyle = '#436a8c'; g.lineWidth = 12; g.lineCap = 'round';
  g.beginPath();
  g.moveTo(44, -4); g.quadraticCurveTo(70, -14 + Math.sin(t * 3) * 8, 78, 12);
  g.stroke();
  g.lineCap = 'butt';
  // 躯干
  eBody(-50, -34, 100, 58, 20, '#9ec4dc', '#436a8c', '#1d3347');
  hazardE(-40, -26, 80, 10);
  // 背上的冰棱
  for (let i = -2; i <= 2; i++) {
    const h = 30 - Math.abs(i) * 6;
    g.fillStyle = 'rgba(206,240,255,0.9)';
    g.beginPath();
    g.moveTo(i * 17 - 8, -34); g.lineTo(i * 17, -34 - h); g.lineTo(i * 17 + 8, -34);
    g.closePath(); g.fill();
    g.strokeStyle = 'rgba(255,255,255,0.8)'; g.lineWidth = 1.4;
    g.beginPath(); g.moveTo(i * 17 - 3, -36); g.lineTo(i * 17, -32 - h); g.stroke();
  }
  // 头：宽吻 + 獠牙
  eBody(-82, -22, 44, 40, 12, '#b0d4e8', '#4d7796', '#213a52');
  g.fillStyle = 'rgba(232,250,255,0.95)';
  for (let i = 0; i < 4; i++) {
    const tx = -78 + i * 10;
    g.beginPath();
    g.moveTo(tx, 14); g.lineTo(tx + 4, 26); g.lineTo(tx + 8, 14);
    g.closePath(); g.fill();
  }
  eEye(-70, -8, 5, '#7ad8ff');
  eEye(-52, -8, 5, '#7ad8ff');
  // 两根弯角
  g.strokeStyle = '#cfeaf8'; g.lineWidth = 7; g.lineCap = 'round';
  for (const sgn of [-1, 1]) {
    g.beginPath();
    g.moveTo(-70 + (sgn > 0 ? 18 : 0), -22);
    g.quadraticCurveTo(-74 + (sgn > 0 ? 34 : 0), -50, -54 + (sgn > 0 ? 24 : 0), -54);
    g.stroke();
  }
  g.lineCap = 'butt';
  // 吐出来的寒气
  if (fxQuality > 0.5) for (let i = 0; i < 4; i++) {
    const ph = (t * 0.9 + i * 0.25) % 1;
    g.fillStyle = 'rgba(200,240,255,' + (0.3 * (1 - ph)).toFixed(3) + ')';
    g.beginPath(); g.arc(-88 - ph * 26, 6 + Math.sin(ph * 6) * 4, 5 + ph * 9, 0, TAU); g.fill();
  }
}

// 虚空之主：悬浮的破碎王座，中间一只巨眼，周围转着裂片
function drawVoidSovereign(e) {
  const t = time, glow = 0.6 + Math.sin(t * 2.2) * 0.3;
  // 外圈转动的裂片
  for (let i = 0; i < 6; i++) {
    const a = t * 0.9 + i * (TAU / 6);
    g.save();
    g.translate(Math.cos(a) * 68, Math.sin(a) * 34);
    g.rotate(a * 1.7);
    g.fillStyle = 'rgba(167,110,232,0.85)';
    g.beginPath();
    g.moveTo(0, -13); g.lineTo(8, 0); g.lineTo(0, 13); g.lineTo(-8, 0);
    g.closePath(); g.fill();
    g.restore();
  }
  // 破碎的王座壳：上下两半分开浮着
  const gap = 4 + Math.sin(t * 1.6) * 2.5;
  eBody(-50, -46 - gap, 100, 40, 14, '#a98ade', '#553396', '#241046');
  eBody(-50, 6 + gap, 100, 36, 14, '#a98ade', '#553396', '#241046');
  hazardE(-40, -40 - gap, 80, 9);
  // 中间的巨眼
  emissive(g, 'rgba(201,138,255,' + glow + ')', 24, () => {
    g.fillStyle = cachedRG(g, 0, 0, 1, 0, 0, 30, [0, '#ffffff', 0.35, '#e8d5ff', 1, '#7a2ecc']);
    g.beginPath(); g.ellipse(0, 0, 30, 19 + Math.sin(t * 3) * 1.5, 0, 0, TAU); g.fill();
  });
  g.fillStyle = '#1a0a30';
  g.beginPath(); g.ellipse(-2 + Math.sin(t * 1.3) * 7, 0, 8, 14, 0, 0, TAU); g.fill();
  g.strokeStyle = 'rgba(232,213,255,0.9)'; g.lineWidth = 2;
  g.beginPath(); g.ellipse(0, 0, 30, 19, 0, 0, TAU); g.stroke();
  // 王冠：一圈悬着的尖片
  for (let i = -3; i <= 3; i++) {
    const h = 20 - Math.abs(i) * 3;
    g.fillStyle = '#ffc531';
    g.beginPath();
    g.moveTo(i * 14 - 5, -50 - gap); g.lineTo(i * 14, -50 - gap - h); g.lineTo(i * 14 + 5, -50 - gap);
    g.closePath(); g.fill();
  }
  // 下摆拖出来的虚空尾迹
  if (fxQuality > 0.5) for (let i = 0; i < 4; i++) {
    const ph = (t * 0.8 + i * 0.25) % 1;
    g.fillStyle = 'rgba(167,110,232,' + (0.26 * (1 - ph)).toFixed(3) + ')';
    g.beginPath(); g.ellipse(0, 44 + gap + ph * 22, 30 * (1 - ph * 0.6), 8, 0, 0, TAU); g.fill();
  }
}
...
/* ========== 调试接口（供自动化测试） ========== */
/* ========== 小游戏：锤机器人 ==========
   和塔防共用画布与指针事件，但完全是另一套状态机：
   state === 'whack' 时主循环走 updateWhack / drawWhack，不碰战场那一套。 */
const WK_COLS = 5, WK_ROWS = 3;
const WK_TIME = 60;
let wk = null;
function wkBest() { try { return +(localStorage.getItem('mg_whackBest') || 0); } catch (e) { return 0; } }
...
function startWhack() {
  ...
  for (let r = 0; r < WK_ROWS; r++) {
    for (let c = 0; c < WK_COLS; c++) {
      holes.push({
        x: 100 + gw * (c + 0.5) / WK_COLS,
        y: 120 + gh * (r + 0.5) / WK_ROWS,
        up: 0,          // 0..1 冒出来的高度
        dir: 0,         // 1 上升 / -1 缩回
        kind: null,     // scrap | armored | repair
        life: 0, hit: 0, flash: 0,
      });
    }
  }
  wk = { t: WK_TIME, score: 0, combo: 0, bestCombo: 0, hits: 0, miss: 0,
         spawnT: 0.6, best: wkBest(), swing: 0, mx: W / 2, my: H / 2, ended: false };
  wk.holes = holes;
  state = 'whack';
  document.body.classList.add('minigame');
  fitStage();
  show('menu', false); show('end', false); show('mini', false); show('clear', false);
  banner('🔨 锤机器人', '冒头就砸 —— 蓝色的维修机器人别砸，砸了扣分');
}

// 三种冒头的目标：分值、停留时间、砸中的反馈都不一样
const WK_KIND = {
  scrap:   { score: 10, dur: [1.15, 0.7], w: 0.55 },
  armored: { score: 25, dur: [0.85, 0.5], w: 0.3 },
  repair:  { score: -30, dur: [1.3, 0.9], w: 0.15 },
};
function wkPick() {
  const r = Math.random();
  return r < 0.55 ? 'scrap' : r < 0.85 ? 'armored' : 'repair';
}
function updateWhack(dt) {
  if (!wk || wk.ended) return;
  wk.t -= dt;
  if (wk.swing > 0) wk.swing -= dt;
  // 越到后面冒得越快、缩得越急
  const prog = 1 - wk.t / WK_TIME;
  wk.spawnT -= dt;
  if (wk.spawnT <= 0) {
    wk.spawnT = Math.max(0.22, 0.85 - prog * 0.55) * rand(0.7, 1.3);
    const idle = wk.holes.filter(h => !h.kind);
    if (idle.length) {
      const h = idle[Math.floor(Math.random() * idle.length)];
      h.kind = wkPick();
      const d = WK_KIND[h.kind].dur;
      h.life = d[0] + (d[1] - d[0]) * prog;
      h.dir = 1; h.up = 0; h.hit = 0;
    }
  }
  for (const h of wk.holes) {
    if (h.flash > 0) h.flash -= dt;
    if (!h.kind) continue;
    if (h.hit > 0) {
      h.hit -= dt;
      h.up = Math.max(0, h.up - dt * 5);
      if (h.hit <= 0) { h.kind = null; h.up = 0; h.dir = 0; }
      continue;
    }
    if (h.dir > 0) {
      h.up = Math.min(1, h.up + dt * 6);
      if (h.up >= 1) { h.dir = 0; }
    } else if (h.dir === 0) {
      h.life -= dt;
      if (h.life <= 0) {
        h.dir = -1;
        if (h.kind !== 'repair') { wk.combo = 0; wk.miss++; }   // 让杂兵溜了：断连击
      }
    } else {
      h.up = Math.max(0, h.up - dt * 4);
      if (h.up <= 0) { h.kind = null; h.dir = 0; }
    }
  }
  if (wk.t <= 0) endWhack();
}
function wkHit(x, y) {
  ...
  let best = null, bd = 1e9;
  for (const h of wk.holes) {
    if (!h.kind || h.hit > 0 || h.up < 0.35) continue;
    const d = Math.hypot(x - h.x, y - (h.y - h.up * 26));
    if (d < 52 && d < bd) { best = h; bd = d; }
  }
  if (!best) { wk.combo = 0; return; }
  const K = WK_KIND[best.kind];
  best.hit = 0.26; best.flash = 0.3;
  if (K.score > 0) {
    wk.combo++;
    wk.bestCombo = Math.max(wk.bestCombo, wk.combo);
    wk.hits++;
    const mul = 1 + Math.min(wk.combo - 1, 9) * 0.2;
    const gain = Math.round(K.score * mul);
    wk.score += gain;
    addFloat(best.x, best.y - 40, '+' + gain + (wk.combo > 1 ? ' ×' + wk.combo : ''), '#ffc531');
    spawnParts(best.x, best.y - 10, '#ffc531', 12, 130, 0.5, 'spark');
    sfx('shred');
  } else {
    wk.combo = 0;
    wk.score = Math.max(0, wk.score + K.score);
    addFloat(best.x, best.y - 40, K.score + ' 砸错了！', '#ff5d5d');
    spawnParts(best.x, best.y - 10, '#4cc2ff', 12, 130, 0.5, 'spark');
    shake(0.2, 4);
    sfx('error');
  }
}
function endWhack() {
  ...
  wk.ended = true;
  state = 'miniover';
  const isBest = wk.score > wk.best;
  if (isBest) { wkSaveBest(wk.score); wk.best = wk.score; }
  $('miniTitle').innerHTML = isBest ? '🏆 <span class="gold">新纪录！</span>' : '🔨 时间到';
  $('miniSub').textContent = '砸中 ' + wk.hits + ' 台 · 漏掉 ' + wk.miss
    + ' 台 · 最长连击 ×' + wk.bestCombo + ' · 最高分 ' + wk.best;
  $('miniScore').textContent = wk.score;
  show('mini', true);
  sfx(isBest ? 'win' : 'lose');
}
function drawWhack() {
  ...
  for (let x = 0; x < W; x += 56) g.fillRect(x, 0, 28, H);
  // 计时条
  const tw = (W - 200) * clamp(wk.t / WK_TIME, 0, 1);
  g.fillStyle = 'rgba(0,0,0,0.45)';
  rr(g, 100, 46, W - 200, 14, 7); g.fill();
  g.fillStyle = wk.t < 10 ? '#ff5d5d' : '#58d68b';
  rr(g, 100, 46, Math.max(2, tw), 14, 7); g.fill();
  g.font = '900 22px sans-serif';
  g.textAlign = 'left';
  g.fillStyle = '#ffc531';
  g.fillText('得分 ' + wk.score, 100, 34);
  g.textAlign = 'right';
  g.fillStyle = '#9fb4c8';
  g.fillText('剩余 ' + Math.ceil(Math.max(wk.t, 0)) + 's', W - 100, 34);
  if (wk.combo > 1) {
    g.textAlign = 'center';
    g.fillStyle = '#ff9d2e';
    g.font = '900 20px sans-serif';
    g.fillText('连击 ×' + wk.combo, W / 2, 34);
  }
  g.textAlign = 'left';
  // 洞 + 冒头的机器人
  for (const h of wk.holes) {
    // 洞
    g.fillStyle = '#0a0e14';
    g.beginPath(); g.ellipse(h.x, h.y, 46, 17, 0, 0, TAU); g.fill();
    g.strokeStyle = '#3a4a5e'; g.lineWidth = 4;
    g.beginPath(); g.ellipse(h.x, h.y, 46, 17, 0, 0, TAU); g.stroke();
    if (!h.kind || h.up <= 0.01) continue;
    const lift = h.up * 52;
    g.save();
    // 用洞口裁一下，冒出来的部分才像是从洞里升上来
    g.beginPath();
    g.rect(h.x - 48, h.y - 124, 96, 124 + 4);
    g.clip();
    g.translate(h.x, h.y + 6 - lift);
    const squash = h.hit > 0 ? 1 + (0.26 - h.hit) * 2.2 : 1;
    g.scale(1, 1 / squash);
    wkDrawBot(h.kind, h.flash > 0);
    g.restore();
    // 洞沿压在身体前面
    g.fillStyle = '#26323f';
    g.beginPath(); g.ellipse(h.x, h.y + 4, 46, 13, 0, 0, TAU); g.fill();
  }
  // 锤子跟着指针
  const sw = wk.swing > 0 ? (0.16 - wk.swing) / 0.16 : 1;
  g.save();
  g.translate(wk.mx + 26, wk.my - 16);
  g.rotate(-0.9 + sw * 0.95);
  g.fillStyle = '#6b5233';
  rr(g, -4, 0, 8, 46, 4); g.fill();
  g.fillStyle = '#8fa3b8';
  rr(g, -20, -16, 40, 22, 6); g.fill();
  g.fillStyle = '#c2d3e2';
  rr(g, -20, -16, 40, 7, 4); g.fill();
  g.fillStyle = '#5c6d80';
  rr(g, -22, -13, 6, 16, 3); g.fill();
  rr(g, 16, -13, 6, 16, 3); g.fill();
  g.restore();
  drawParts();
  drawFloats();
}
...
function wkDrawBot(kind, flash) {
  if (kind === 'repair') {
    eBody(-24, -34, 48, 44, 12, '#bfe9ff', '#3f7a9c', '#1e3d52');
    g.fillStyle = '#4cc2ff';
    rr(g, -14, -26, 28, 9, 4); g.fill();
    eEye(-9, -10, 5, '#ffffff');
    eEye(9, -10, 5, '#ffffff');
    // 头顶的十字灯：一眼看出来是自己人
    emissive(g, 'rgba(88,214,139,0.95)', 12, () => {
      g.fillStyle = '#58d68b';
      rr(g, -4, -50, 8, 20, 3); g.fill();
      rr(g, -10, -44, 20, 8, 3); g.fill();
    });
  } else if (kind === 'armored') {
    eBody(-27, -36, 54, 48, 10, '#9fb2c6', '#4c5f74', '#232f3d');
    hazardE(-22, -30, 44, 9);
    eEye(-10, -12, 5.5, '#ff5d5d');
    eEye(10, -12, 5.5, '#ff5d5d');
    g.fillStyle = '#6b7c90';
    for (const sgn of [-1, 1]) {
      g.beginPath();
      g.moveTo(sgn * 26, -30); g.lineTo(sgn * 40, -36); g.lineTo(sgn * 26, -16);
      g.closePath(); g.fill();
    }
  } else {
    eBody(-22, -32, 44, 42, 11, '#c8d4e0', '#5f6f80', '#2a3543');
    eEye(-8, -12, 5, '#ff9d2e');
    eEye(8, -12, 5, '#ff9d2e');
    g.fillStyle = '#7f8ca0';
    rr(g, -3, -46, 6, 14, 3); g.fill();
    g.fillStyle = '#ffc531';
    g.beginPath(); g.arc(0, -48, 4, 0, TAU); g.fill();
  }
  if (flash) {
    g.fillStyle = 'rgba(255,255,255,0.55)';
    rr(g, -28, -50, 56, 62, 12); g.fill();
  }
}
...
window.__game = {
  ...
  playCard: (t, r, c) => {
    if (state !== 'playing' || boxMode()) return false;
  ...
  padAt: (r, c) => hasPad(r, c),
  shovelPad: (r, c) => { if (!hasPad(r, c)) return false; pads[r][c] = false; return true; },
  kingHold: () => kingHoldX(),
  get campStyle() { return campStyle; },
  get campDiff() { return campDiff; },
  setDiff: (k) => { if (DIFFS[k]) { campDiff = k; saveDiff(); } },
  get unlockedAll() { return allCardsUnlocked(); },
  ...
  laneAt: (r) => laneOf(r),
  terrainAt: (r, c) => terrainAt(r, c),
  rowOk: (t, r, c) => rowAllows(t, r, c),
  nextOf: (m, st) => nextStage(m, st),
  ...
  makeDummy: (i, hp, x) => {
    ...
    e.hp = e.maxHp = hp; e.speed = 0; e.x = x === undefined ? W - 120 : x;
