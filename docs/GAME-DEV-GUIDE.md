...
### 3.1 ★ 多地图：把「地形」压成一张表里的一个字符串

要做「水池 / 海滩 / 天空」这种关卡变化时，最容易走的歪路是给每张图写一套逻辑。
正确的做法是：**每张图只多一个字段，字段里每个字符描述一行**，剩下的全部由这一个字段推导。

```js
const MAPS = {
  pool: { name: '水池', lanes: 'GWWWG',      // G=地面 W=水 S=悬空
          sky: [...], cell: [...], water: [...],   // 配色也放表里
          boss: 'sharklord', tip: '中间三行是水……' },
};
const laneOf = r => MAPS[curMap].lanes[r] || 'G';
```

然后三处规则**全部**只读这一个字符串，不写第二份地形知识：

| 规则 | 实现 | 一行代码 |
| --- | --- | --- |
| 谁能进这一行 | 刷怪时过滤行号 | `rowPassable(info, r)`：水行只放会游/会飞的 |
| 玩家能放什么 | `place()` 里挡一道 | `rowAllows(type, r)`：地雷/钉刺沉底，水和空放不了 |
| 背景怎么画 | `buildBackground()` 里按行分支 | `'W'` 画水面＋焦散，`'S'` 画浮空石台＋云 |

配色也一并进表（`sky` / `cell` / `wall` / `water` / `foam`），`buildBackground()` 里一个 `mapDef()` 取出来用，
**换图 = 换一行数据 + `bgCanvas = null`**，不用改渲染代码。

#### 一开始就按「格」建模，不要按「行」

第一版把地形做成了「每行一种」（`lanes: 'GWWWG'`）。等到要做一条**竖着的水道**时，
整套规则（能不能放、走不走得了、怎么画）全要重写一遍。正确的形状是从一开始就：

```js
function terrainAt(r, c) { ... }        // 唯一入口，所有规则都问它
function terrainAtX(r, x) { return terrainAt(r, colAtX(x)); }
```

`lanes`（整行）和 `waterCols`（整列）都只是**填这张表的两种写法**，规则一律读 `terrainAt`。
背景绘制则按行扫一遍、把连着的同种地形合成一段来画，水面才不会一格一段地断开。

还有一条设计上的教训：**竖着的地形不能用「不能通过」来做**。
一条竖水道如果挡住地面单位，那一行就彻底走不通，整张图直接废掉。
改成「**能过，但被拖慢**」（`wadeMul()`），它就从一堵墙变成了一个天然的屠宰场——
这才是玩家想要的那种地形。

美术上再补一条：**同一个单位在不同地形要有不同的"接地表现"**，
否则玩家会觉得机器"站在水面上"。本项目的三种：

- 地面 → 椭圆投影
- 水里 → 不落影，改成一圈向外推开的涟漪；机器脚下加一只浮筒
- 悬空 → 本来走路的单位脚下点两束推进焰，说明它是被反重力托着的
...
纯代数很好扩展，但**全都是并列，没有惊喜**。在合并函数里留一张小小的配方表，
让特定几对塌缩成全新的东西，性价比极高：

```js
const ELEMENT_FUSION = [
  [['frost', 'flame'],  'obsidian'],    // 冰 + 火 → 黑曜石
  [['zap',   'flame'],  'plasma'],
  [['zap',   'frost'],  'stormfrost'],
  [['poison','flame'],  'corrosion'],
];
// mergeModules 里，求和之后：
for (const [[k1, k2], out] of ELEMENT_FUSION) {
  if (map[k1] && map[k2]) {
    map[out] = (map[out] || 0) + map[k1] + map[k2];   // 等级取两者之和
...
1. **等级取和，不取平均**：「冰 3 + 火 2 = 黑曜 5」才让人愿意去凑，取平均就没人玩了。
2. **只塌缩一轮**：产物不再参与二次融合。否则合成路径会变成一棵没人能预测的树，
   玩家也说不清自己手里这台是怎么来的。
3. **配方表放在通用算法里面，不要放在外面**：这样杂交预览、命名、造型、
   AI 机器人全都自动跟着走，一处都不用改。
4. **让配方产物当「附魔」而不是「本体」**：排序时把元素一律排到普通模块之后，
   于是「碎纸机 + 黑曜」自然成了**黑曜石碎纸机**（碎纸机的身子 + 黑玻璃质感），
   而不是「绞碎黑曜石炮」。玩家的直觉是「给我的机器附上一个属性」，不是「换一台机器」。

   ```js
   const ELEM_TIER = { obsidian: 1, plasma: 1, /* … */ voidglass: 2, /* … */ };
   mods.sort((a, b) =>
     (ELEM_TIER[a.kind] || 0) - (ELEM_TIER[b.kind] || 0)   // 元素永远靠后
     || b.lv - a.lv || KIND_ORDER.indexOf(a.kind) - KIND_ORDER.indexOf(b.kind));
   ```
   一行排序规则，命名、机体造型、染色、外挂件、徽章全都跟着对了。

   还要再走一步：**附魔不能长得像挂件**。通用系统会给每个副模块挂一个圆徽章、
   一件外挂零件——元素套上这套，玩家看到的就还是「一台普通杂交机」。
   把元素从徽章位和挂件位里摘出去，改成一层直接长在机体上的**材质**
   （染色 + 少数几笔硬边细节），它才会被读成「这台机器是黑曜石做的」。
   判断标准很简单：玩家说的是「我的路障变成黑曜石的了」，还是「我的路障上多了个圈」。

   后来这条推到了全部能力：**徽章圆圈整个删掉**，每种能力都补一件长在机体上的实体结构
   （拳击＝侧挂弹簧拳、锯轮＝转着的锯片、钉刺＝底边尖钉、引力＝脚下旋涡……）。
   代价是要给每种能力都画一件东西，收益是玩家一眼看出「我的炮台现在近战远程都能打」。
6. **附魔要看宿主是干什么的**。同一个元素，落在炮台上就是一门炮，落在纯防御机体上
   就该是一堵更硬的墙——玩家不会想要「一堵会开炮的墙」。判定很简单：
   身上除元素外全是被动模块（装甲/护盾/折射/自愈/超频/充能/钉刺）就切「护体」形态，
   元素不开火，改成耐久倍率 + 被咬时的反击。只要有一个主动攻击模块，就恢复原样。
5. **二级配方别复制六份逻辑**：六种「奇点」共用同一套打法（贯穿整行的一束），
   差别写成一张状态表（碎裂 / 灼烧 / 冻结 / 麻痹 / 剧毒 / 融盾），造型也共用一套轮廓、
   靠核的形状与配色区分。**六个新东西，一条代码路径**——这样二级配方才做得起。

代价只有：每个产物要补齐那 8 张表 + 一个造型函数。四个配方 ≈ 半天工作量，
但玩家得到的是「原来还能这样」的发现感——这是纯代数给不了的。
...
### 11.1 ★ 「敌人在出生点就被秒」——塔防最常见的手感问题

症状：玩家说"没有挑战性"。根因通常**不是数值不够，而是没有前线**。

先量，别猜。记录每个敌人**死在第几列**：

```js
if (killLog) killLog.push({ x: Math.round(e.x), col: Math.floor((e.x - GRID_X) / CELL_W) });
```

本项目量出来：**66% 的敌人在走进战场之前就死了**，96% 死在最右三列，
第 5 列往左从来没被踏足过——难怪没有危机感。

三个结构性原因，按重要性排：

1. **射程无限**。所有塔都能打整行 ⇒ 摆位毫无意义，全场火力永远压在入口。
   把射程做成模块属性（并**随等级成长**），前线立刻由摆位决定。
   效果有多大：同一个机器人，只把炮台从第 1~3 列挪到第 1~5 列，胜率 **0/4 → 4/4**。
2. **出生即可被攻击**。给一条「战场边界」，越过才算上场，双方都不能隔场边对射。
   在 `damageEnemy` 一处加闸即可覆盖所有伤害来源，不用改几十个索敌点。
3. **敌人排队来**。一个一个放 ⇒ 全场火力永远集中在同一个目标上。
   改成成群放（间隔按人数同比拉长，总量不变），火力被分散，前线才推得进来。

⚠️ 加了防御射程，就必须同时给远程敌人**压迫推进**（站桩越久，停火距离越短），
否则射程比你远的远程兵会永远停在场外对射，**波次清不掉 = 软锁**，比太简单更糟。

⚠️ 改了核心规则，**机器人策略要同步更新**。本项目一度以为难度爆表（杂交流 0/4），
实际是机器人还在按"列号无所谓"的旧规则把塔堆在后排。

衡量指标建议同时看三个：**每波耗时**、**同屏敌人峰值**、**最深突入列**。
健康的曲线长这样（波次越后，前线越往基地退）：
...
### 11.2 ★ 关卡制的难度曲线：进度只能乘在「关内进度」上

把 10 波一局改成「4 张图 × 4 关 × 5 波」之后，第一版的公式是这样的：

```js
const pow = mapIndex * 4 + (stage - 1);      // 总推进度 0..15
const k = 2 + Math.floor(pow * 0.5) + n;     // ← 错在这里
```

机器人台架的结果是**四张图的第 1 关全部第一波就输**。原因很直白：
**每一关都从空场地 + 150 起始能量重新开始**，但 `pow` 让第 4 张图的第 1 波直接有 12 个敌人。
关卡等级不能直接加在波次强度上，它只能**乘以关内进度**：

```js
const ramp = n / STAGE_WAVES;                       // 0.2 → 1.0
pick(light, Math.round(1 + n + lvl * 0.28 * ramp));  // 第 1 波几乎不受 lvl 影响
if (n >= 2) pick(tough, ...);                        // 硬货一律从第 2 波起
```

再加一条：**基地被摸到就直接结束**的游戏，第 1 波只能放杂兵。
把 `armored / shieldbot / splitter` 从「第 1 波可选池」里拿掉之后，
非 Boss 关的机器人胜率从 ~60% 回到 22/24，而 Boss 关稳定卡在 3/8——这正是想要的形状。

| 关型 | 目标胜率（中等机器人） |
| --- | --- |
| 普通关 | 90%+，是给玩家攒阵型的 |
| 关底 Boss 关 | 30~50%，要输几次才过 |

### 11.3 难度选项要调「血量和数量」，不要调速度

三档难度（轻松 / 普通 / 困难）只动三个系数：敌人血量、每波数量、得分倍率。
**速度一律不动**——提速会直接改掉整个游戏的手感和摆位逻辑，
玩家会觉得「困难模式是另一个游戏」，而不是「同一个游戏更难」。

八图 × 抽查三关的机器人台架结果：轻松 23/24、普通 21/24、困难 18/24，
且困难下越到后面的地图越顶不住——这就是想要的形状。
...
### 12.1 测量要"撑住负载"，否则测的是空场

压力场景里单位会被打光，等你开始计时时场上已经没人了。**必须在测量窗口里持续补敌**：

```js
setInterval(() => { for (let k = G.enemyCount; k < 40; k++) G.spawn('titan', k % 5); }, 120);
```

本项目就吃过这个亏：同一个场景，不补敌测出 45 FPS，补敌后只有 5 FPS。

### 12.2 先 profile，再优化

不要靠猜。用 CDP 抓一份 CPU profile，按 self time 排序：
...
1. **音效先看一眼**：合成音效（噪声缓冲、WebAudio 节点）在密集触发时非常贵。
   噪声缓冲按时长分档缓存复用；同名音效设最小重触发间隔（50~90ms）——
   一帧里响一百次和响一次听起来是一样的
2. `shadowBlur` 是 Canvas 2D 单项最贵的操作。密集时直接关掉，
   改用**双描边假光晕**（粗而淡一圈 + 细而亮一圈），观感接近，代价只有两次普通描边
3. 去掉 `save/clip/restore`（很贵）：无朝向的圆形粒子直接按绝对坐标画，别 translate+rotate
4. 渐变缓存，但**注意 key 的基数**
5. `fxQuality` 分级降级细节
6. **降渲染分辨率**是最后也最有效的一档（见 12.4）
7. 最后才考虑离屏 canvas 缓存

### 12.4 自适应画质：按帧率而不是按单位数

单位数只是负载的估算，弱机（尤其手机）单位不多也可能吃力。做成两层：

```js
const ref = Math.min(fpsBest, 62);                       // 参照系＝这台设备自己最好的成绩
const slow = fpsAvg < ref * 0.72 || fpsAvg < 26;         // 相对判据 + 绝对下限
const easy = fpsAvg > ref * 0.9  && fpsAvg > 40;
if (slow) fxLoad = Math.max(0.35, fxLoad - 0.02);
else if (easy) fxLoad = Math.min(1, fxLoad + 0.008);
fxQuality = Math.min(单位数档位, fxLoad);
```

**阈值不要写死 60**：30Hz 屏、省电模式、后台标签页都会被永久判成卡顿，画面白白变素。

特效降到底还跟不上，就降渲染分辨率（`cv.width = W * DPR`，CSS 尺寸不变，
所以布局和点击坐标都不受影响）。**必须走离散档位 + 冷却**，因为改分辨率要重建背景缓存——
本项目一开始用连续微调，每几帧重建一次背景，反而比不优化还慢（19 → 17 FPS）；
改成 4 档 + 3 秒冷却后是 34 FPS。

### 12.5 红线：性能降级不能改变玩法

弹幕/技能上限**不要跟画质挂钩**。写成 `bullets.length < (fxQuality < 0.7 ? 120 : 260)`
看起来很合理，实际是"掉帧就削弱玩家"——把性能问题转嫁成了难度问题。
上限只该由"同屏还看得清多少发"决定。

同理，任何为了省性能而改的数值（攻速、齐射数、间隔）都要**回归跑一遍机器人**。
本项目把齐射间隔拉长 + 单发伤害同比放大，理论 DPS 不变，
实测杂交机器人胜率却从 3/4 掉到 1/4 —— 因为加重的单发打在杂兵身上全是**溢出伤害**。
最后只对最高档（4 连发）收这个代价，低档原样保留，胜率才回到 3/4。

**用定量的 DPS 台架代替机器人胜率来定位这类问题**（机器人方差太大，3 局根本看不出来）：

```js
G.spawn('titan', 2); G.makeDummy(0, 1e9);   // 定住不动、清掉护盾/词缀的血包
G.advance(10);
const dps = (1e9 - G.enemyState(0).hp) / 10;
```

同时测两个指标：**对高血目标的 DPS**（看真实输出）和**对杂兵的击杀数**（看溢出浪费）。
...
| 13 | **压力测试里单位被打光** | 测出的 FPS 虚高 | 测量窗口内持续补敌 |
| 14 | **音效成了性能瓶颈** | 密集开火时掉一半帧 | 噪声缓冲缓存复用 + 同名音效设最小重触发间隔 |
| 15 | **连续微调渲染分辨率** | 反而比不优化更慢 | 离散档位 + 换档冷却，别每几帧重建一次背景缓存 |
| 16 | **降级逻辑影响了玩法** | 掉帧顺带削弱玩家 | 弹幕/技能上限不跟 `fxQuality` 挂钩 |
| 17 | **"DPS 不变"的改动实际变弱了** | 机器人胜率掉一半 | 加重单发会在杂兵身上产生溢出浪费；用 DPS 台架量化，别只看胜率 |
| 18 | **测伤害的血包不掉血** | DPS 恒为 0 | 血包要顺手清掉护盾 / 隐匿 / 词缀减伤 |
| 19 | **敌人在出生点就被打死** | 没有前线、没有危机感 | 出生保护 + 给防御设射程；见 11.1 |
| 20 | **防御射程 < 远程敌人射程** | 波次永远清不掉（软锁） | 远程敌人加「压迫推进」：站桩越久越往前压 |
| 21 | **机器人用旧规则打分** | 误判游戏太难 | 改了核心规则要同步更新机器人策略，否则测的是「机器人不会玩」 |
| 22 | **新模式复用老系统，但老系统里写死了 `mode === 'x'`** | 战役模式下每种卡只能放一台，之后永远灰着 | 冷却递减那行是 `if (mode === 'classic')`。**加模式时全局 grep 一遍 `mode ===`**，把该带上的都带上 |
| 25 | **把「地形」建成每行一种** | 要做竖着的水道时，三处规则全得重写 | 一开始就 `terrainAt(r, c)`，`lanes` / `waterCols` 只是填表的两种写法；见 3.1 |
| 26 | **竖着的地形做成「不能通过」** | 那一行彻底走不通，整张图废掉 | 改成「能过但被拖慢」，地形从墙变成屠宰场 |
| 27 | **测试里写死 `COLS/ROWS/W/H`** | 战场一加大，十来个用例集体假失败 | 调试接口暴露 `geom`，测试一律从它取几何 |
| 28 | **给玩家做的池子忘了分敌我** | 岩浆喷口喷出来的岩浆在烧自己的机器 | 复用敌方机制做玩家武器时，先找那段代码里「伤害谁」的那一行 |
| 29 | **加了解锁进度，机器人台架却没解锁** | 机器人只剩 10 张卡，胜率暴跌，误判成平衡崩了 | 台架启动时先把进度写进 localStorage，再开始测 |
| 23 | **关卡等级直接加到第 1 波强度上** | 后面几张图的第 1 关开局就输 | 每关都是空场地重开，关卡等级只能乘 `n / 总波数`；见 11.2 |
| 24 | **新模式的文案落进了 else 分支** | 战役开局提示「点开盲盒」 | `if (creative()) … else if (classic) … else 盲盒` 这种链子，加模式时要补分支而不是靠兜底 |
...
| --- | --- |
| `game.js` | ~15,100 行 |
| `index.html` | ~760 行（含全部 CSS） |
| `worker.js` | ~105 行 |
| 战场 | 11 列 × 6 行，逐格地形（地面 / 水 / 悬空） |
| 己方单位 | 54 种 × 3 级造型 + 无限杂交组合 |
| 敌人 | 53 种（含远程、融合、8 个关底 Boss） |
| 地图 / 关卡 | 8 张 × 10 关 |
| 运行时依赖 | 0 |
| 图片资源 | 0 |
| 测试脚本 | 20+ 个 |
