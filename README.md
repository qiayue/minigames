...
| 🃏 普通模式 | 经典种卡：全部 10 种机器一开始就在卡槽，按标价用能量放置，每张卡有独立冷却 |

两种模式共用能量经济、波次、合成与排行榜。
...
- **🧤 手套**：点"移动"按钮，选中一台机器再点空格把它搬过去（冷却 5 秒），随时调整阵型
...
**任意两台机器都能杂交**，产物还能继续杂交。每台机器最多可叠加 **5 个能力模块**，每个模块最高 3 级：

- **同类杂交 → 升级**：自动炮台＋自动炮台＝双管炮台，双管炮台＋双管炮台＝**加特林炮台**；
  每条线都有三级进化（发电机→聚变电站→核能核心、碎纸机→工业碎纸机→湮灭粉碎机……共 10 条进化线）
- **异类杂交 → 叠能力**：碎纸机＋路障＝装甲碎纸机，再＋发电机＝充能装甲碎纸机……一路叠到**五能力全能机器人**；
  能力自动命名成修饰词链（如 充能装甲冰霜绞碎特斯拉线圈），机体边缘挂对应的副能力徽章
- **超出上限**：第 6 种能力加入时自动保留最强的 5 个；同能力等级相加、上限 3 级
- **模块协同**：带雷电模块的炮弹变成电弧弹跳弹，带冰霜模块的炮弹自带减速；冰霜 2 级起会轰出**冻结整行**的冰冻炮弹
- **经典组合**保留专属名字和造型：电弧机炮、磁力碎纸机、寒冰壁垒、冰冻弹簧炮（冻结整行 2 秒）
- 合成度越高机体越华丽：金圈 → 橙色光环 → 紫色光环
