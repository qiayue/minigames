...
| 👑 神位争夺 | 所有机器开局即 **99999 级**、炮台加特林式连射；神机**要花钱**（5 倍价、无冷却），能量靠**手点电池**攒；**两尊同阶神合成上一阶神**；僵尸血量同样 99999，敌潮三倍量 |
...
- **开局即神**：一落地就是 **99999 级**——但**长着一级机器的样子**，叫「一级神·自动炮台」
- **神机很贵，不送**：价格是普通模式的 **5 倍**（发电机 250⚡、自动炮台 500⚡、特斯拉线圈 1250⚡），
  开局 1200⚡。没有冷却——**唯一的节流阀是你的手速**
- **攻速拉满，但弹体还是一级的样子**：炮台每 **0.035~0.07 秒**一发，
  泼出去的是一串小能量弹（不是一颗大炮弹），单发伤害 99999，普通僵尸一枪一个
- **产能：电池照样自己点**：电池本身没变（还是普通的 25⚡，二级神 40⚡、三级神 60⚡），
  变的是**出得多快**——每 **0.2 秒**一颗，朝右前方弹散开，场上最多同时存在 42 颗、5 秒不捡就消失。
  神阶提升的是**电池面值**而不是速度，所以「把发电机合上去」比「再造一台」划算
- **僵尸也会咬人**：血量 99999 的僵尸，啃咬伤害同步拉到 **7142**（约 14 口啃穿一尊一级神）。
  否则一只溜到防线身后的杂兵能对着神机啃半小时——炮弹只朝右飞，谁也够不着它
...
| 音爆 | 2.6 / 3.2 / 3.8 | 近距击退 |
| 烈焰 | 1.6 / 2.0 / 2.4 | 贴脸高频 |
| 狙击 / 激光 / 轰爆 | 整行 | 长射程反制，专门用来打站远处的远程兵 |
| 重拳 / 锯轮 / 绞碎 / 钉刺 | 近身 | 接触触发 |
...
- **状态可读**：灼烧（橙色轮廓 + 上跳火苗）、中毒（绿色轮廓 + 上浮气泡）、冻结（冰块封印）
  一眼可辨，且不会因为叠加而糊成一团
- **性能**：三层自适应保证高等级 / 神位模式不掉帧 —— ①渐变按内容缓存复用、
  辉光用「双描边假光晕」代替昂贵的 `shadowBlur`；②画质随场上单位数**和实时帧率**浮动
  （阈值按设备自身最好成绩算，30Hz 屏和省电模式不会被误判）；③特效降到底还跟不上时
  自动降渲染分辨率（离散档位 + 冷却，避免来回抖）。
  另外弹幕、碎屑、冲击波、飘字都有上限，同名音效有最小重触发间隔（几十台神机齐射时，
  光是合成白噪声就能吃掉两成 CPU）。
  实测：极端压力场景（45 台神机 + 持续 40 敌）9fps → 38fps，
  Lv12 炮台阵 5fps → 40fps，手机端神位模式 11fps → 34fps；
