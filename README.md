...
战役模式里还有**水池、落日海滩、天空之城**三张地形不同的图，每张图最后一关压轴一个会召唤小 Boss 的关底 Boss。
...
| 🃏 普通模式 | 经典种卡：全部 29 种机器一开始就在卡槽，按标价用能量放置，每张卡有独立冷却 |
| 🗺️ 战役 | **四张地图 × 四关**，逐关解锁，每张图最后一关压轴一个专属关底 Boss——见下一节 |
| 🛠️ 创造模式 | 能量无限、机器免费无冷却、随便放随便杂交，敌潮一键开关，**双击 Lv3+ 机器直接复制、长按调等级**——沙盒实验场 |

四种模式共用波次、杂交与手套；盲盒与普通模式各有独立排行榜，创造模式不计成绩，
战役进度存在浏览器本地（`localStorage` 的 `mg_campaign`）。

## 🗺️ 战役：四张地图 · 十六关 · 四个关底 Boss

![战役关卡选择](docs/campaign.png)

从菜单点 **🗺️ 战役** 展开关卡表。第一张图的第 1 关默认解锁，**通一关解锁下一关，通完一张图解锁下一张图**。
每关 5 波，第 4 关的最后一波是这张图的关底 Boss。

### 地形：每张图的五条通道不一样

一行是「地面 / 水 / 悬空」中的一种，这一条规则同时决定了**谁能过来**和**你能放什么**：

| 地图 | 通道 | 地形规则 | 关底 Boss |
| --- | --- | --- | --- |
| 🏭 废铁厂 | 地地地地地 | 老地方，五条都是实地 | ☠ 泰坦之王 |
| 🌊 水池 | 地水水水地 | 中间三行是水：**只有会游、会飞的过得来**；地雷与钉刺沉底放不了 | ☠ 深渊鲨王 |
| 🏖️ 落日海滩 | 地地地水水 | 下面两行是浅滩，鲨群从浪里来，沙地上蟹兵横行 | ☠ 巨钳蟹将 |
| ☁️ 天空之城 | 空空空空空 | 全是悬空平台：**所有敌人都会飞**（连废铁兵都被反重力托着走），地雷与钉刺没处放 | ☠ 雷霆母舰王 |

![水池](docs/map-pool.png)

放在水行上的机器会自动踩上一只**浮筒**；泡在水里的敌人不落影子，改成推开一圈涟漪；
在天空图上被反重力托起来的地面单位，脚下会点两束推进焰——地形该怎么读，看一眼就知道。

### 关底 Boss：会生小 Boss 的那种

![四个关底 Boss](docs/bosses.png)

| Boss | 血/盾 | 特点 |
| --- | --- | --- |
| 泰坦之王 | 9000 / 1600 | 戴王冠的重装巨像，三枚炉心，双臂重锤；**每 9 秒召唤 2 台重型碾压车** |
| 深渊鲨王 | 8200 / 1200 | 会游泳的装甲鲨，铰接下颌 + 喉咙红光；背上两根鱼雷管，**每 8 秒放出 2 条机械鲨** |
| 巨钳蟹将 | 9600 / 1800 | 最厚的一个，双巨钳；**每 7 秒召唤 3 只钳兵蟹** |
| 雷霆母舰王 | 7800 / 1400 | **会飞**的四旋翼母舰，4.2 格外三连齐射；**每 7 秒放出 2 架雷暴无人机** |

关底 Boss 比普通泰坦大一圈，脚下压着一圈红色威压光环，头顶挂着带名字的血条 + 护盾条，
冰冻/电击/击退对它们统统无效或减半。
...
| 敌人 | 特点 |
| --- | --- |
| 机械鲨 | 520 血的半潜艇身，**只走水路**，背鳍划水 |
| 水雷水母 | 飘在水里的自爆水雷，撞上机器 **320 伤害**同归于尽 |
| 深潜射手 | 水下远程，3.2 格外喷酸液 |
| 钳兵蟹 | 沙滩杂兵，横着走，成群出现 |
...
### 机器图鉴（共 29 种，另有 10 种只能靠杂交得到的元素机）
...
### 敌人图鉴（共 32 种，另有 4 种融合敌人与 4 个战役关底 Boss）
